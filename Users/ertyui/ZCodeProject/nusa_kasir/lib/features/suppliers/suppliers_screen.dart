...
class _SuppliersScreenState extends ConsumerState<SuppliersScreen> {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              if (mounted) {
                Navigator.of(context).pop();
                TopToast.success(context, 'Supplier dihapus');
              }
              _load();
            },
  ...
  Future<void> _exportCSV() async {
    try {
      ...
      final file = File(
        '${dir.path}/supplier_nusa_${DateTime.now().millisecondsSinceEpoch}.csv',
      );
      await file.writeAsString(csv);
      await Share.shareXFiles([
        XFile(file.path),
      ], text: 'Daftar Supplier NUSA Kasir');
    } catch (e) {
  ...
  Future<void> _exportPDF() async {
    try {
      ...
      for (final s in _list) {
        buf.writeln(
          '${s.name} | ${s.phone ?? '-'} | ${s.address ?? '-'} | ${s.contactPerson ?? '-'} | ${s.note ?? '-'}',
        );
      }
      buf.writeln('=' * 60);
      ...
      final dir = await getTemporaryDirectory();
      final file = File(
        '${dir.path}/supplier_nusa_${DateTime.now().millisecondsSinceEpoch}.txt',
      );
...
class _SupplierTile extends StatelessWidget {
  ...
  _SupplierTile({required this.supplier, required this.onTap});
  ...
  Color _avatarColor() =>
      _avatarColors[supplier.name.hashCode.abs() % _avatarColors.length];
            ...
            children: [
              ...
              if (hasPhone)
                PopupMenuButton<String>(
                  ...
                  itemBuilder: (_) => [
                    PopupMenuItem(value: 'tel', child: Text('Telepon')),
