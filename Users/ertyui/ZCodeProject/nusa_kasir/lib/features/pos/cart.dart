...
class CartNotifier extends StateNotifier<List<CartItem>> {
  ...
  void addProduct(int productId, String name, int price,
      {String? note, double? weightKg, int? originalPrice, int qty = 1}) {
    ...
    if (i >= 0) {
      ...
      state = [...state, CartItem(productId: productId, name: name, price: price,
        originalPrice: originalPrice, note: note, weightKg: weightKg, qty: qty)];
    }
  }
  ...
  void addManualItem(String name, int price, {int qty = 1}) {
    _manualSeq--;
    addProduct(_manualSeq, name, price, qty: qty);
  }
