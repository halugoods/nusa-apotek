...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  // ── Cloud Google Sheet (v2.2.57+123) ───────────────────────────
  // Per-user Google Drive backup — sambungkan akun sendiri, simpan cadangan
  // langsung ke Google Drive user tanpa setup client id/secret/dashboard.

  Future<void> _showCloudGoogleSheet() async {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final connected = await CloudGoogleService.isConnected();
    final email = await CloudGoogleService.connectedEmail();
    final lastAt = await CloudGoogleService.lastUploadedAt();
          ...
          return Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  'Simpan cadangan database ke Google Drive Anda sendiri',
                  style: TextStyle(
                    fontSize: 13,
                    color: isDark
                        ? NusaConfig.darkTextSecondary
                ...
                if (connected) ...[
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (lastAt != null) ...[
                          const SizedBox(height: 4),
                          Text(
                            'Terakhir diupload: ${CloudGoogleService.formatDate(lastAt)}',
                            style: TextStyle(
                              fontSize: 12,
                              color: isDark
                                  ? NusaConfig.darkTextTertiary
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Setiap backup otomatis disalin ke Google Drive Anda. '
                            'File tersimpan sebagai NUSA_Backup_${NusaConfig.productId}.sqlite.enc '
                            'di folder utama Drive.',
                            style: TextStyle(
                              fontSize: 12,
                              height: 1.4,
                              color: isDark
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: () async {
                        await CloudGoogleService.I.disconnect();
                        setSt(() {});
                        if (mounted) Navigator.pop(ctx);
                        TopToast.success(
                          context,
                          'Cloud Google diputuskan',
                        );
                      },
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                          'Sambungkan Google Drive untuk menyimpan '
                          'cadangan secara otomatis di akun Anda sendiri.',
                          textAlign: TextAlign.center,
                          style: TextStyle(
                            fontSize: 12,
                            height: 1.4,
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () async {
                        final ok = await CloudGoogleService.I.connect();
                        if (!mounted) return;
                        if (ok) {
                          setSt(() {});
                          TopToast.success(
                      ...
                      label: const Text('Sambungkan Google Drive'),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: NusaConfig.activePrimary,
                        foregroundColor: Colors.white,
                        shape: RoundedRectangleBorder(
          ...
          children: [
            ...
            _groupCard(
              ...
              tiles: [
                ...
                // v2.2.57+123: Cloud Google — per-user Drive backup (simpan
                // salinan cadangan di Google Drive milik user sendiri).
                FutureBuilder<bool>(
                  future: CloudGoogleService.isConnected(),
                  builder: (ctx, snap) {
                    ...
                    return _menuTile(
                      ...
                      title: 'Cloud Google',
                      subtitle: connected
                          ? 'Backup otomatis tersimpan di Google Drive Anda'
                          : 'Sambungkan Google Drive untuk menyimpan cadangan',
                      isDark: isDark,
                      onTap: () => _showCloudGoogleSheet(),
                      trailing: connected
                          ? Container(
                              padding: const EdgeInsets.symmetric(
                                horizontal: 8,
