...
class CartItem {
  ...
  CartItem({required this.productId, required this.name, required this.price,
    this.originalPrice, this.costPrice, this.qty = 1, this.note, this.weightKg});
...
class CartNotifier extends StateNotifier<List<CartItem>> {
  ...
  void addProduct(int productId, String name, int price,
      {String? note, double? weightKg, int? originalPrice, int? costPrice, int qty = 1}) {
    final i = state.indexWhere((e) => e.productId == productId);
    if (i >= 0) {
      final list = [...state];
      list[i] = CartItem(productId: productId, name: name, price: price,
        originalPrice: list[i].originalPrice, costPrice: list[i].costPrice ?? costPrice,
        qty: list[i].qty + qty, note: list[i].note ?? note, weightKg: list[i].weightKg ?? weightKg);
      state = list;
    } else {
      state = [...state, CartItem(productId: productId, name: name, price: price,
        originalPrice: originalPrice, costPrice: costPrice, note: note, weightKg: weightKg, qty: qty)];
    }
  }
  void changeQty(int productId, int delta) {
    ...
    if (nq <= 0) list.removeAt(i); else list[i] = CartItem(productId: productId, name: list[i].name, price: list[i].price, originalPrice: list[i].originalPrice, costPrice: list[i].costPrice, qty: nq, note: list[i].note, weightKg: list[i].weightKg);
    state = list;
  }
  ...
  void setNote(int productId, String? note) {
    ...
    if (i < 0) return;
    final list = [...state];
    list[i] = CartItem(productId: list[i].productId, name: list[i].name, price: list[i].price, originalPrice: list[i].originalPrice, costPrice: list[i].costPrice, qty: list[i].qty, note: note, weightKg: list[i].weightKg);
    state = list;
  }
  ...
  void setWeight(int productId, double? weightKg) {
    ...
    if (i < 0) return;
    final list = [...state];
    list[i] = CartItem(productId: list[i].productId, name: list[i].name, price: list[i].price, originalPrice: list[i].originalPrice, costPrice: list[i].costPrice, qty: weightKg == null ? list[i].qty : 1, note: list[i].note, weightKg: weightKg);
    state = list;
  }
