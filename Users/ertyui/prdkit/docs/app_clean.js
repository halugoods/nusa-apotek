/* =============================
   PRDKit Ã¢â‚¬â€ Application Logic
   Polished & Maintainable SPA
   ============================= */

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ SPA Hash Router Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
const PAGES = ['home', 'setup', 'wizard', 'result'];
let currentPage = 'home';
let techPref = 'gratis'; // 'gratis' or 'advanced'

// Alias: navigate() Ã¢â€ â€™ router.navigate()
...
class PRDKitRouter {
  ...
  resolve() {
    ...
    if (!this.init) {
      this.init = true;
      // First resolve Ã¢â‚¬â€ also do page-specific init
  ...
  showPage(page, params) {
    ...
    // Page-specific init (skip on initial load Ã¢â‚¬â€ done in resolve)
    if (this.init) {
      if (page === 'home') {
...
// Legacy navigateTo Ã¢â‚¬â€ now delegates to hash change
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Saved Provider Selection Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
/**
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ API Key Security Module Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
const KEY_STORE = {
  ...
  set(key) {
    this._key = key || '';
    // Store obfuscated (reverse + base64 Ã¢â‚¬â€ NOT encryption, just not plaintext)
    try {
      if (key) {
  ...
  display() {
    if (!this._key) return '';
    return this._key.substring(0, 4) + 'Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢' + this._key.slice(-4);
  },
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ First Visit Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ AI Config (persisted via Worker + localStorage) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
async function loadAIConfig() {
  try {
    ...
    // Load from Worker (D1) Ã¢â‚¬â€ with credentials so cookies are sent
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (!data.savedWithUserId) {
        console.warn('Ã¢Å¡Â Ã¯Â¸Â Config saved GLOBALLY (no user auth)');
      }
    } else {
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Auth: Google OAuth Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Tech Options Data Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
const TECH_OPTIONS = {
  frontend: [
    ['ai-pilih', 'Biarkan AI pilih', false],
    ['nextjs', 'Next.js', true],
    ['react-vite', 'React + Vite', true],
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loadHomeStats();
  // Tap glow Ã¢â‚¬â€ only on touch devices; desktop stays normal hover
  if ('ontouchstart' in window || navigator.maxTouchPoints > 0) {
    ...
    document.addEventListener('touchstart', function(ev) {
      var card = ev.target.closest('.card, .cat-card, .sub-card, .mode-card');
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Business Categories Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
const CATEGORIES = [
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ State Persistence Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function saveState() {
  try {
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Toast Notifications Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ First-Time Experience Utils Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Analytics (top level Ã¢â‚¬â€ always accessible) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
const TIPS = [
  'Ã°Å¸â€™Â¡ Tips: Jelaskan model bisnis Anda dengan detail agar AI menghasilkan blueprint yang lebih akurat.',
  'Ã°Å¸â€™Â¡ Tips: Sebutkan target pengguna (customer, merchant, admin) agar entity dan role tergenerate otomatis.',
  'Ã°Å¸â€™Â¡ Tips: Tambahkan informasi monetisasi agar AI bisa generate API payment yang sesuai.',
  'Ã°Å¸â€™Â¡ Tips: Jelaskan workflow utama agar business flow di prompt lebih relevan.',
  'Ã°Å¸â€™Â¡ Tips: Setup AI Provider dulu (Settings) sebelum generate blueprint.',
  'Ã°Å¸â€™Â¡ Tips: Hasil generate bisa di-copy langsung ke Claude Code, Cursor, atau ChatGPT.',
];
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Adaptive Survey Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
const SURVEY_DEPTH = {
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Survey Recommendations Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ AI Survey Generator Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
async function generateSurveyQuestions(mode) {
  ...
  const catStr = catParts.length ? catParts.join(' Ã¢â€ â€™ ') : '-';
  ...
  var prompt = [
    ...
    modeDesc,
    '',
    'Ã¢Å¡Â Ã¯Â¸Â PENTING: Buat ' + qCount + ' pertanyaan (jangan kurang, jangan lebih).',
    'Hitung dengan teliti jumlah array sebelum return.',
    '',
    'Contoh: untuk ' + qCount + ' pertanyaan, return array dengan ' + qCount + ' object.',
    '',
    'Aturan:',
    '- Pertanyaan HARUS SPESIFIK untuk domain ' + domain + ' Ã¢â‚¬â€ jangan generic',
    ...
    '- Urut: base/modal bisnis Ã¢â€ â€™ operational Ã¢â€ â€™ technical Ã¢â€ â€™ monetisasi/scale',
  ...
  try {
    ...
    // Fallback: generate static questions based on mode Ã¢â‚¬â€ not just 4 generic ones
...
// Mode selector Ã¢â‚¬â€ triggered when user clicks a mode card in Step 3
...
// Survey recommendation chip click Ã¢â‚¬â€ appends to text input
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ (OLD Wizard system removed Ã¢â‚¬â€ NEW system lives in index.html) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Step 1: Ide Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Step 2: Teknologi Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
function updateTechHeaderSelected(cat) {
  ...
  if (val === 'ai-pilih') {
    btn.innerHTML = 'Pilih Ã¢â€“Â¶';
  } else {
    const options = TECH_OPTIONS[cat] || [];
    const found = options.find(o => o[0] === val);
    btn.innerHTML = (found ? found[1] : val) + ' Ã¢Å“â€œ';
  }
}
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Step 3: Survey Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    ${qs.map((q, i) => {
      ...
      return `<div class="survey-question" data-q="${i}"${hidden}>
        ...
        <div class="survey-q-nav">
          <button class="survey-nav-btn" id="surveyPrevBtn" onclick="prevSurvey()"${i === 0 ? ' style="visibility:hidden"' : ''}>Ã¢â€ Â Sebelumnya</button>
          <button class="survey-nav-btn accent" id="surveyNextBtn" onclick="nextSurvey()">${i === qs.length - 1 ? 'Lanjut ke Teknologi Ã¢â€ â€™' : 'Selanjutnya Ã¢â€ â€™'}</button>
        </div>
      </div>`;
    }).join('')}
  `;
...
function nextSurvey() {
  ...
  if (state.surveyQ < total - 1) {
    ...
    if (navBtn) navBtn.textContent = 'Lanjut Ã¢â€ â€™';
  } else {
    // Survey done Ã¢â‚¬â€ go to teknologi step
    goWizardStep(3);
  }
}
...
function updateSurveyNav() {
  ...
  btn.innerHTML = state.surveyQ >= state.surveyTotal - 1
    ? 'Lanjut ke Teknologi Ã¢â€ â€™'
    : `Lanjut ${iconSvg('arrowRight', 14)}`;
}

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Domain Detection (Hybrid AI + Keyword) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function hasTerm(text, terms) {
  const lower = text.toLowerCase();
  return terms.some((term) => lower.includes(term));
}

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ DOMAIN PACKS Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
const DOMAIN_PACKS = {
  delivery: {
    ...
    flows: [
      'Merchant menerima order via WhatsApp Ã¢â‚¬â€ staff create order di sistem',
      'System assigns driver terdekat yang available Ã¢â‚¬â€ driver dapat notifikasi',
      'Driver accept Ã¢â€ â€™ status berubah ke ON_THE_WAY Ã¢â‚¬â€ customer lihat tracking realtime',
      'Driver sampai lokasi Ã¢â€ â€™ marked DELIVERED Ã¢â‚¬â€ customer konfirmasi',
      'Payment diverifikasi Ã¢â‚¬â€ order complete Ã¢â‚¬â€ merchant dan driver dapat ringkasan',
    ],
    endpoints: [
  ...
  inventory: {
    ...
    flows: [
      'Staff gudang menerima barang baru Ã¢â‚¬â€ catat batch, lokasi rak, dan quantity',
      'Sistem validasi SKU unik Ã¢â‚¬â€ jika produk baru, buat master produk dulu',
      'Stok otomatis bertambah Ã¢â‚¬â€ jika melebihi batas, tidak perlu alert',
      'Staff mengeluarkan stok Ã¢â‚¬â€ validasi qty tersedia Ã¢â‚¬â€ jika kurang, error + sisa qty',
      'Sistem cek low stock setiap transaksi keluar Ã¢â‚¬â€ jika di bawah min, create alert',
  ...
  crm: {
    ...
    flows: [
      'Sales mendapat lead baru Ã¢â‚¬â€ create lead dengan source dan score awal',
      'Sales melakukan follow-up Ã¢â‚¬â€ catat activity (call/email/meeting) Ã¢â‚¬â€ update status',
      'Lead qualified Ã¢â€ â€™ create deal dengan value dan expected close date',
      'Deal berjalan melalui pipeline stages Ã¢â‚¬â€ update probability setiap stage',
      'Deal WON Ã¢â€ â€™ generate invoice Ã¢â‚¬â€ kirim ke customer Ã¢â‚¬â€ tunggu payment',
  ...
  booking: {
    ...
    flows: [
      ...
      'Sistem menampilkan slot yang available Ã¢â‚¬â€ customer pilih waktu',
      'Booking dibuat dengan status PENDING Ã¢â‚¬â€ slot diblokir 15 menit',
      'Staff/admin konfirmasi booking Ã¢â‚¬â€ status jadi CONFIRMED Ã¢â‚¬â€ customer dapat notif',
      'H-1: sistem kirim reminder otomatis Ã¢â‚¬â€ jika no response, booking bisa dibatalkan',
      'Selesai layanan Ã¢â‚¬â€ staff mark COMPLETED Ã¢â‚¬â€ customer bisa rating',
    ],
    endpoints: [
  ...
  finance: {
    ...
    flows: [
      'User mencatat transaksi baru Ã¢â‚¬â€ pilih tipe (income/expense), kategori, nominal, tanggal',
      'Sistem update saldo account otomatis Ã¢â‚¬â€ validasi nominal > 0',
      'Jika expense melebihi budget kategori Ã¢â‚¬â€ sistem beri alert',
      'Akhir bulan Ã¢â‚¬â€ sistem generate ringkasan cashflow + laporan laba rugi sederhana',
      'User dapat export laporan ke CSV/PDF dengan rentang tanggal',
      'Auditor dapat merekonsiliasi transaksi Ã¢â‚¬â€ tandai isReconciled = true',
    ],
    endpoints: [
  ...
  commerce: {
    ...
    flows: [
      'Buyer browsing katalog Ã¢â‚¬â€ filter by kategori, search, sort by harga/terbaru',
      'Buyer add to cart Ã¢â‚¬â€ bisa ubah quantity Ã¢â‚¬â€ validasi stok',
      'Checkout Ã¢â‚¬â€ pilih alamat pengiriman Ã¢â‚¬â€ pilih metode pembayaran Ã¢â‚¬â€ submit order',
      'Sistem validasi stok Ã¢â‚¬â€ kurangi stock Ã¢â‚¬â€ buat order dengan status PENDING',
      'Seller terima notifikasi Ã¢â‚¬â€ process order Ã¢â‚¬â€ update status ke SHIPPED + input resi',
      'Buyer terima barang Ã¢â‚¬â€ konfirmasi Ã¢â‚¬â€ rating & review produk',
    ],
    endpoints: [
  ...
  habit: {
    ...
    flows: [
      'User membuat habit baru Ã¢â‚¬â€ tentukan nama, frekuensi (harian/mingguan), dan target per minggu',
      'Setiap hari Ã¢â‚¬â€ user check-in dengan satu tap Ã¢â‚¬â€ sistem catat timestamp dan update streak',
      'Jika user melewatkan 1 hari (harian) Ã¢â‚¬â€ streak terputus Ã¢â‚¬â€ streak saat ini disimpan sebagai best attempt',
      'Akhir minggu Ã¢â‚¬â€ sistem generate insight: konsistensi, streak, dibanding minggu sebelumnya',
      'User lihat progress calendar Ã¢â‚¬â€ hijau (selesai), abu-abu (terlewat), angka streak',
    ],
    endpoints: [
  ...
  laundry: {
    ...
    flows: ['Customer datang ke toko atau order via WhatsApp Ã¢â‚¬â€ staff catat order', 'Staff menimbang dan mencatat item laundry Ã¢â‚¬â€ sistem generate total harga', 'Laundry masuk ke proses: washing Ã¢â€ â€™ drying Ã¢â€ â€™ ironing Ã¢â€ â€™ packing', 'Customer dapat tracking status laundry via WhatsApp atau website', 'Selesai Ã¢â‚¬â€ customer bayar dan ambil laundry (atau diantar driver)', 'Dashboard owner: order hari ini, pendapatan, status processing'],
  ...
  pos: {
    ...
    flows: ['Customer belanja Ã¢â‚¬â€ cashier scan barcode produk', 'Sistem menampilkan nama produk dan harga Ã¢â‚¬â€ cashier input quantity', 'Checkout Ã¢â‚¬â€ sistem hitung total, cashier input nominal bayar', 'Sistem hitung kembalian Ã¢â‚¬â€ cetak struk (thermal printer)', 'Stok otomatis berkurang Ã¢â‚¬â€ jika stok minimal, muncul notifikasi', 'Akhir shift Ã¢â‚¬â€ cashier tutup kasir, sistem hitung total penjualan shift'],
  ...
  erp: {
    ...
    flows: ['Admin mendaftarkan department dan employee baru', 'Employee mengajukan leave Ã¢â‚¬â€ manager approve/reject', 'Finance mengelola budget per department Ã¢â‚¬â€ tracking pengeluaran', 'Manager assign task ke employee Ã¢â‚¬â€ employee update status', 'Asset dicatat dan dilacak lokasinya Ã¢â‚¬â€ maintenance dijadwalkan', 'Dashboard executive: summary per department, budget utilization, headcount'],
  ...
  manufacturing: {
    ...
    flows: ['Production Manager membuat production order Ã¢â‚¬â€ tentukan produk, quantity, deadline', 'Operator memulai produksi Ã¢â‚¬â€ sistem mencatat start time', 'Material diambil dari gudang Ã¢â‚¬â€ sistem kurangi stok material', 'Produksi selesai Ã¢â‚¬â€ QC menginspeksi hasil: pass/reject', 'Produk jadi masuk gudang Ã¢â‚¬â€ stok produk jadi bertambah', 'Dashboard: order aktif, produksi hari ini, reject rate'],
  ...
  healthcare: {
    ...
    flows: ['Pasien daftar (online/offline) Ã¢â‚¬â€ receptionist create appointment', 'Pasien check-in Ã¢â‚¬â€ sistem notifikasi dokter', 'Dokter panggil pasien Ã¢â‚¬â€ consultation Ã¢â‚¬â€ diagnosis Ã¢â‚¬â€ resep', 'Pasien bayar di kasir Ã¢â‚¬â€ sistem generate invoice', 'Medical record tersimpan Ã¢â‚¬â€ bisa diakses untuk follow-up', 'Dashboard: pasien hari ini, pendapatan, jadwal dokter'],
  ...
  education: {
    ...
    flows: ['Admin membuat course dengan lesson dan materi', 'Student mendaftar course Ã¢â‚¬â€ enrollment aktif', 'Student mengakses lesson Ã¢â‚¬â€ sistem catat progress', 'Student submit assignment/quiz Ã¢â‚¬â€ sistem nilai otomatis atau manual', 'Course selesai Ã¢â‚¬â€ certificate generated', 'Teacher lihat dashboard: student progress, nilai, completion rate'],
  ...
  property: {
    ...
    flows: ['Owner/Agent mendaftarkan properti dan unit', 'Tenant cari properti Ã¢â‚¬â€ lihat unit tersedia', 'Tenant pilih unit Ã¢â‚¬â€ buat lease agreement', 'Tenant bayar sewa bulanan Ã¢â‚¬â€ sistem catat payment', 'Maintenance request Ã¢â‚¬â€ owner assign tukang', 'Dashboard: occupancy rate, revenue, pending maintenance'],
  ...
  restaurant: {
    ...
    flows: ['Customer places order Ã¢â‚¬â€ Waiter takes order Ã¢â‚¬â€ Chef receives KitchenTicket and cooks', 'Waiter serves food to table', 'Customer requests bill Ã¢â‚¬â€ Waiter processes payment', 'Manager reviews daily sales and reservations', 'Dashboard: orders, revenue, top menu items, table occupancy'],
  ...
  cafe: {
    ...
    flows: ['Customer arrives and orders at counter', 'Cashier takes order Ã¢â‚¬â€ system prints order to barista', 'Barista prepares drinks and food', 'Barista marks order as ready Ã¢â‚¬â€ customer picks up', 'Customer pays at cashier Ã¢â‚¬â€ loyalty points earned', 'Dashboard: daily sales, popular items, customer visits'],
  ...
  bakery: {
    ...
    flows: ['Baker plans production based on daily demand forecast', 'Baker produces goods in batches Ã¢â‚¬â€ system records production', 'Fresh products displayed on shelves with updated stock', 'Customer buys products Ã¢â‚¬â€ Cashier processes sale', 'System auto-generates restock alerts for low stock items', 'End of day Ã¢â‚¬â€ production vs sales report'],
  ...
  catering: {
    ...
    flows: ['Customer requests quote Ã¢â‚¬â€ Admin recommends package based on pax and event type', 'Customer selects package Ã¢â‚¬â€ Admin sends proposal and confirms order', 'Chef prepares menu items based on order specifications', 'Driver delivers to event location on schedule', 'Customer confirms receipt Ã¢â‚¬â€ Payment processed', 'Dashboard: upcoming events, revenue, menu popularity'],
  ...
  hotel: {
    ...
    flows: ['Guest makes reservation Ã¢â‚¬â€ Receptionist confirms booking and assigns room', 'Guest checks in Ã¢â‚¬â€ Receptionist verifies ID and collects payment', 'Guest stays Ã¢â‚¬â€ Housekeeping cleans room daily', 'Guest checks out Ã¢â‚¬â€ Receptionist processes final payment', 'Housekeeping inspects room and updates status to available', 'Manager reviews occupancy, revenue, and housekeeping efficiency'],
  ...
  salon: {
    ...
    flows: ['Customer books appointment Ã¢â‚¬â€ selects service and stylist', 'Customer arrives Ã¢â‚¬â€ Receptionist confirms appointment', 'Stylist performs service Ã¢â‚¬â€ system tracks time and materials', 'Customer pays at counter', 'Customer leaves review Ã¢â‚¬â€ system records for loyalty program', 'Manager reviews daily performance and stylist productivity'],
  ...
  barbershop: {
    ...
    flows: ['Customer walks in or books appointment online', 'Customer waits in queue Ã¢â‚¬â€ Barber calls next customer', 'Barber performs service (haircut, shave, etc.)', 'Customer pays at counter Ã¢â‚¬â€ cash or digital', 'Owner reviews daily earnings, barber performance, and walk-in vs booking ratio'],
  ...
  workshop: {
    ...
    flows: ['Customer datang Ã¢â‚¬â€ technician terima dan catat keluhan', 'Technician lakukan diagnosa Ã¢â‚¬â€ sistem buat estimasi biaya dan waktu', 'Customer setuju estimasi Ã¢â‚¬â€ work order approved', 'Technician kerjakan service Ã¢â‚¬â€ update status tiap tahap', 'Service selesai Ã¢â‚¬â€ customer bayar dan ambil kendaraan', 'Manager review: work order aktif, revenue, technician performance'],
  ...
  cleaning_service: {
    ...
    flows: ['Customer orders cleaning service Ã¢â‚¬â€ selects service type and schedule', 'Admin assigns cleaner based on availability and location', 'Cleaner arrives at location Ã¢â‚¬â€ starts job with status update', 'Cleaner completes service Ã¢â‚¬â€ customer verifies quality', 'Customer pays Ã¢â‚¬â€ leaves rating and review', 'Admin reviews completed jobs and cleaner performance'],
  ...
  field_service: {
    ...
    flows: ['Customer submits service request with description and location', 'Dispatcher reviews and assigns technician based on skills and proximity', 'Technician receives notification Ã¢â‚¬â€ departs to customer location', 'Technician arrives on site Ã¢â‚¬â€ starts service', 'Technician completes service Ã¢â‚¬â€ customer signs off', 'Payment processed Ã¢â‚¬â€ customer rates service quality'],
  ...
  invoicing: {
    ...
    flows: ['Finance creates invoice for customer Ã¢â‚¬â€ system generates invoice number', 'Invoice is sent to customer via email', 'System sends automated reminders as due date approaches', 'Customer makes payment Ã¢â‚¬â€ system records and reconciles', 'Finance marks invoice as paid Ã¢â‚¬â€ updates accounting', 'Admin generates aging report for overdue invoices'],
  ...
  expense: {
    ...
    flows: ['Employee submits expense report with receipt image', 'Manager reviews and approves/rejects expense', 'Approved expense goes to Finance for reimbursement', 'Finance processes payment Ã¢â‚¬â€ marks as reimbursed', 'System categorizes expense for budgeting reports', 'Month-end Ã¢â‚¬â€ Finance exports expense report by category'],
  ...
  budgeting: {
    ...
    flows: ['Department Head proposes annual budget with line items', 'Manager and Finance review and approve budget', 'Budget becomes active Ã¢â‚¬â€ departments can spend against it', 'Actual spending tracked against budget items in real-time', 'Quarterly review Ã¢â‚¬â€ Finance adjusts forecast based on actuals', 'Year-end Ã¢â‚¬â€ Budget closed, variance analysis report generated'],
  ...
  attendance: {
    ...
    flows: ['Employee checks in at start of shift Ã¢â‚¬â€ system records timestamp', 'System tracks working hours and calculates overtime', 'Employee submits leave request Ã¢â‚¬â€ Manager approves/rejects', 'Employee checks out Ã¢â‚¬â€ system calculates total hours', 'HR reviews monthly attendance report', 'Payroll system uses attendance data for salary calculation'],
  ...
  recruitment: {
    ...
    flows: ['HR posts job opening on multiple platforms', 'Candidates apply Ã¢â‚¬â€ system collects resumes and cover letters', 'HR screens applications Ã¢â‚¬â€ shortlists qualified candidates', 'Interviewer schedules and conducts interviews', 'HR extends offer to selected candidate', 'Candidate accepts Ã¢â‚¬â€ onboarding process begins'],
  ...
  employee_management: {
    ...
    flows: ['HR creates employee profile with personal and employment details', 'Employee is assigned to department and position', 'Employee uploads required documents (KTP, NPWP, BPJS, etc.)', 'Manager can initiate transfer or promotion for employee', 'HR processes resignation/termination Ã¢â‚¬â€ offboarding workflow', 'Dashboard: headcount, department distribution, document completeness'],
  ...
  performance_management: {
    ...
    flows: ['Manager and employee set quarterly goals together', 'Employee works on goals Ã¢â‚¬â€ updates progress regularly', 'Mid-year check-in Ã¢â‚¬â€ Manager provides informal feedback', 'End of year Ã¢â‚¬â€ Manager completes performance review', 'Employee acknowledges review Ã¢â‚¬â€ discusses development plan', 'HR aggregates results for promotion and compensation decisions'],
  ...
  fleet: {
    ...
    flows: ['Fleet Manager assigns vehicle and driver for a trip', 'Driver starts trip Ã¢â‚¬â€ system records odometer and time', 'GPS tracking monitors vehicle location during trip', 'Trip ends Ã¢â‚¬â€ driver logs end odometer and fuel usage', 'Scheduled maintenance alerts for upcoming service', 'Reports: fuel efficiency, maintenance costs, trip history'],
  ...
  courier: {
    ...
    flows: ['Sender creates shipment Ã¢â‚¬â€ system generates tracking number', 'Courier picks up package from sender location', 'Package arrives at sorting facility Ã¢â‚¬â€ scanned and sorted by destination', 'Package loaded for transit to destination city', 'Local courier picks up for last-mile delivery', 'Package delivered Ã¢â‚¬â€ receiver signs Ã¢â‚¬â€ status updated'],
  ...
  trucking: {
    ...
    flows: ['Shipper books trucking service for load', 'Dispatcher assigns truck and driver to the load', 'Driver loads cargo at pickup location', 'Driver departs Ã¢â‚¬â€ GPS tracking enabled', 'Driver delivers cargo at destination Ã¢â‚¬â€ receiver signs', 'Dispatch confirms delivery Ã¢â‚¬â€ payment processed'],
  ...
  digital_product: {
    ...
    flows: ['Creator uploads digital product with description and pricing', 'Admin reviews and publishes product', 'Customer browses marketplace Ã¢â‚¬â€ searches by category', 'Customer purchases product Ã¢â‚¬â€ instant download access', 'Customer receives license key (if applicable)', 'Creator receives payment Ã¢â‚¬â€ dashboard shows sales analytics'],
  ...
  course_platform: {
    ...
    flows: ['Instructor creates course with lessons and learning materials', 'Admin reviews and publishes course', 'Student browses and enrolls in course Ã¢â‚¬â€ payment processed', 'Student accesses lessons Ã¢â‚¬â€ progress tracked automatically', 'Student completes course Ã¢â‚¬â€ receives certificate', 'Student leaves review Ã¢â‚¬â€ instructor gets feedback'],
  ...
  farm: {
    ...
    flows: ['Farmer plants crops in designated fields', 'Worker tends to crops Ã¢â‚¬â€ applies fertilizer and water', 'Crops mature Ã¢â‚¬â€ Farmer schedules harvest', 'Worker harvests crops Ã¢â‚¬â€ records yield and quality', 'Harvest is stored in inventory or sold to buyers', 'Dashboard: active crops, harvest yields, sales revenue'],
  ...
  livestock: {
    ...
    flows: ['New animal registered with tag number and type', 'Farmer records daily feeding for each animal', 'Vet performs regular health checkups and vaccinations', 'Breeding program Ã¢â‚¬â€ male and female paired', 'Animal sold Ã¢â‚¬â€ Farmer records sale details', 'Dashboard: animal count, health status, breeding success'],
  ...
  poultry: {
    ...
    flows: ['Farmer receives new flock Ã¢â‚¬â€ registers with breed and quantity', 'Worker feeds and waters daily Ã¢â‚¬â€ records mortality and growth', 'Vaccination schedule followed for disease prevention', 'Daily egg collection recorded (for layers)', 'Birds reach market weight Ã¢â‚¬â€ sold to buyers', 'Dashboard: mortality rate, feed conversion, revenue per flock'],
  ...
  bengkel: {
    ...
    flows: ['Customer datang Ã¢â‚¬â€ Mekanik terima kendaraan dan catat keluhan', 'Mekanik diagnosa masalah Ã¢â‚¬â€ buat estimasi biaya dan waktu', 'Customer setuju estimasi Ã¢â‚¬â€ Service order approved', 'Mekanik kerjakan perbaikan Ã¢â‚¬â€ catat spare part digunakan', 'Service selesai Ã¢â‚¬â€ Customer bayar dan ambil kendaraan', 'Owner review: revenue, spare part stok, mekanik performance'],
  ...
  car_rental: {
    ...
    flows: ['Customer browses available vehicles and pricing', 'Customer books vehicle for specific dates Ã¢â‚¬â€ Staff confirms', 'Customer picks up vehicle Ã¢â‚¬â€ Staff inspects and hands over keys', 'Customer returns vehicle Ã¢â‚¬â€ Staff inspects for damage', 'Payment processed Ã¢â‚¬â€ deposit refunded if no issues', 'Manager reviews fleet utilization and revenue'],
  ...
  contractor: {
    ...
    flows: ['Client and Contractor sign contract Ã¢â‚¬â€ project created', 'Architect provides design and specifications', 'Contractor creates task breakdown and assigns workers', 'Materials ordered and delivered to site', 'Workers complete tasks Ã¢â‚¬â€ progress updated daily', 'Project completed Ã¢â‚¬â€ final inspection and handover'],
  ...
  maintenance: {
    ...
    flows: ['Tenant reports maintenance issue with description and priority', 'Manager reviews and creates work order Ã¢â‚¬â€ assigns technician', 'Technician schedules visit and performs repair', 'Technician marks work order as completed with notes', 'Tenant confirms issue resolved Ã¢â‚¬â€ work order closed', 'Manager reviews response time and recurring issues'],
  ...
  event_management: {
    ...
    flows: ['Organizer creates event with details and ticket tiers', 'Event published Ã¢â‚¬â€ attendees can register and purchase tickets', 'Speaker confirmed for sessions Ã¢â‚¬â€ schedule published', 'Attendee checks in at venue Ã¢â‚¬â€ badge printed', 'Event executes Ã¢â‚¬â€ sessions, networking, activities', 'Post-event Ã¢â‚¬â€ feedback collected, attendance report generated'],
  ...
  forum: {
    ...
    flows: ['User browses categories and threads', 'User creates new thread with title and content', 'Other users reply with posts and discussions', 'Users can upvote/downvote helpful posts', 'Moderator reviews reports Ã¢â‚¬â€ pins, locks, or deletes content', 'Admin manages categories and user roles'],
  ...
  photography: {
    ...
    flows: ['Client browses photography packages and pricing', 'Client books a session Ã¢â‚¬â€ selects package and date', 'Photographer shoots on scheduled date', 'Photographer edits photos and uploads to gallery', 'Client reviews gallery and approves final delivery', 'Payment processed Ã¢â‚¬â€ Client receives high-res files'],
  ...
  veterinary: {
    ...
    flows: ['PetOwner registers pet and books appointment', 'Receptionist checks in pet on arrival', 'Vet examines pet Ã¢â‚¬â€ diagnosis and treatment plan', 'Vet prescribes medication if needed', 'PetOwner pays at reception', 'Follow-up scheduled if necessary'],
  ...
  gym: {
    ...
    flows: ['Guest registers as member Ã¢â‚¬â€ selects membership plan', 'Member attends gym Ã¢â‚¬â€ checks in at reception', 'Member joins fitness classes with trainers', 'System tracks attendance and workout frequency', 'Membership renewal Ã¢â‚¬â€ payment processed', 'Admin reviews member retention and class attendance'],
  ...
  coworking: {
    ...
    flows: ['Guest browses available spaces and pricing plans', 'Member books a space (desk/room) for specific time', 'Member checks in Ã¢â‚¬â€ Staff confirms booking', 'Member uses the space with amenities', 'Member checks out Ã¢â‚¬â€ Staff inspects if needed', 'Payment processed Ã¢â‚¬â€ member leaves review'],
  ...
  pharmacy: {
    ...
    flows: ['Pharmacist checks stock levels for medicines', 'Customer brings prescription Ã¢â‚¬â€ pharmacist validates', 'Pharmacist dispenses medicine Ã¢â‚¬â€ stock decremented', 'Sale recorded Ã¢â‚¬â€ payment processed', 'Admin reconciles daily sales and stock adjustments'],
  ...
  laboratory: {
    ...
    flows: ['Patient registers for lab test Ã¢â‚¬â€ selects test panel', 'LabTech collects sample from patient at scheduled time', 'Sample is processed and analyzed in the lab', 'Results are verified and published for doctor review', 'Doctor interprets results and shares with patient'],
  ...
  telemedicine: {
    ...
    flows: ['Patient books an appointment with a doctor online', 'Doctor accepts consultation Ã¢â‚¬â€ video/chat session begins', 'Doctor diagnoses patient and writes prescription', 'Prescription sent to patient Ã¢â‚¬â€ payment processed', 'Follow-up scheduled if needed Ã¢â‚¬â€ medical record updated'],
  ...
  tutoring: {
    ...
    flows: ['Parent registers student and selects subjects', 'Student books tutoring session with a tutor', 'Tutor conducts the session Ã¢â‚¬â€ teaches and assigns homework', 'Student completes homework Ã¢â‚¬â€ tutor reviews and gives feedback', 'Admin generates monthly report and processes payments'],
  ...
  insurance: {
    ...
    flows: ['Client requests a quote Ã¢â‚¬â€ agent assesses risk', 'Agent submits application Ã¢â‚¬â€ policy is issued', 'Client pays premium Ã¢â‚¬â€ policy becomes active', 'Client files a claim Ã¢â‚¬â€ adjuster investigates', 'Claim is approved or rejected Ã¢â‚¬â€ settlement processed'],
  ...
  cold_chain: {
    ...
    flows: ['Operator loads temperature-sensitive products into vehicle', 'Sensors begin monitoring temperature during transit', 'System alerts if temperature exceeds thresholds', 'Shipment delivered Ã¢â‚¬â€ temperature logs verified', 'Manager reviews chain integrity report'],
  ...
  freight: {
    ...
    flows: ['Shipper requests a freight quote with weight and destination', 'Carrier provides pricing Ã¢â‚¬â€ shipper books shipment', 'Carrier picks up cargo from shipper', 'Cargo is in transit Ã¢â‚¬â€ tracking events recorded', 'Cargo delivered Ã¢â‚¬â€ payment processed and documents finalized'],
  ...
  homestay: {
    ...
    flows: ['Host lists property with photos and pricing', 'Guest searches and books a property for specific dates', 'Guest checks in Ã¢â‚¬â€ host welcomes and provides keys', 'Guest stays and enjoys amenities', 'Guest checks out Ã¢â‚¬â€ host inspects property', 'Payment released to host Ã¢â‚¬â€ guest leaves review'],
  ...
  villa_rental: {
    ...
    flows: ['Guest searches for villas by location and dates', 'Guest books a villa Ã¢â‚¬â€ payment required for confirmation', 'Guest pays deposit Ã¢â‚¬â€ booking is confirmed', 'Guest checks in and enjoys the villa', 'Guest checks out Ã¢â‚¬â€ owner inspects property', 'Remaining payment processed Ã¢â‚¬â€ guest reviews'],
  ...
  guest_house: {
    ...
    flows: ['Guest searches room availability and makes reservation', 'Receptionist confirms reservation and checks guest in', 'Guest stays and can request additional services', 'Guest checks out Ã¢â‚¬â€ receptionist processes payment', 'Payment settled and guest record updated'],
  ...
  resort: {
    ...
    flows: ['Guest books a room for specific dates', 'Guest arrives Ã¢â‚¬â€ receptionist checks in and assigns room', 'Guest enjoys resort activities and spa', 'Staff provides housekeeping and room service', 'Guest checks out Ã¢â‚¬â€ all charges are settled', 'Guest provides feedback and review'],
  ...
  help_desk: {
    ...
    flows: ['Customer submits a support ticket with issue details', 'System assigns ticket to available agent based on category', 'Agent responds to customer and works on resolution', 'Agent marks ticket as resolved Ã¢â‚¬â€ customer confirms', 'Ticket is closed Ã¢â‚¬â€ satisfaction survey sent'],
  ...
  sales_pipeline: {
    ...
    flows: ['Sales generates leads from multiple sources', 'Sales qualifies leads through discovery calls', 'Sales creates proposals and sends to prospects', 'Sales negotiates terms and handles objections', 'Deal is won Ã¢â‚¬â€ contract signed and handoff to delivery'],
  ...
  project_management: {
    ...
    flows: ['Manager creates project with timeline and team', 'Manager assigns tasks to team members', 'Members update task progress and log hours', 'Manager reviews timesheets and task completion', 'Project completed Ã¢â‚¬â€ deliverables archived and report generated'],
  ...
  okr_tracking: {
    ...
    flows: ['Manager sets quarterly objectives and key results', 'Teams align their OKRs with company objectives', 'Users track progress with weekly check-ins', 'Manager reviews progress during mid-quarter', 'Scores calculated at end of quarter Ã¢â‚¬â€ results documented'],
  ...
  content_subscription: {
    ...
    flows: ['Creator publishes exclusive content for subscribers', 'Subscriber signs up for a paid subscription plan', 'Subscriber gains access to exclusive content', 'Creator gets paid Ã¢â‚¬â€ subscription revenue analytics', 'Creator analyzes subscriber growth and revenue trends'],
  ...
  legal: {
    ...
    flows: ['Client submit legal request Ã¢â‚¬â€ admin create case file', 'Lawyer review case dan assign task', 'Paralegal upload evidence dan supporting documents', 'Lawyer update status sampai closure', 'Client receive progress update dan final outcome'],
  ...
  plantation: {
    ...
    flows: ['Farmer prepares land and plants crops', 'Worker maintains crops Ã¢â‚¬â€ watering, fertilizing, pest control', 'Crops grow and are monitored for readiness', 'Harvest is collected and sorted by quality', 'Produce is processed and sold to buyers'],
  ...
  car_wash: {
    ...
    flows: ['Customer arrives at car wash', 'Customer selects service package', 'Worker washes the car Ã¢â‚¬â€ interior and exterior', 'Car is dried and quality checked', 'Customer pays and leaves'],
  ...
  motorcycle_workshop: {
    ...
    flows: ['Customer brings motorcycle with a problem', 'Mechanic diagnoses the issue and provides estimate', 'Customer approves estimate Ã¢â‚¬â€ repair begins', 'Mechanic repairs and replaces parts as needed', 'Customer pays and picks up motorcycle'],
  ...
  tire_shop: {
    ...
    flows: ['Customer arrives for tire inspection', 'Technician inspects tires Ã¢â‚¬â€ checks tread depth and pressure', 'Technician replaces worn tires with new ones', 'Tires are balanced and aligned', 'Customer pays and drives away'],
  ...
  real_estate_agency: {
    ...
    flows: ['Agent lists a property with photos and details', 'Client schedules a property showing', 'Agent shows the property to interested buyers', 'Buyer submits an offer Ã¢â‚¬â€ agent negotiates', 'Deal closes Ã¢â‚¬â€ agent receives commission'],
  ...
  tailoring: {
    ...
    flows: ['Customer consults with tailor about desired garment', 'Tailor takes customer measurements', 'Tailor cuts fabric and sews the garment', 'Customer comes for fitting Ã¢â‚¬â€ adjustments made', 'Garment is completed and delivered to customer'],
  ...
  convenience_store: {
    ...
    flows: ['Cashier starts shift and opens register', 'Customer buys items Ã¢â‚¬â€ cashier scans and processes payment', 'Cashier handles cash or digital payment', 'Stock is updated after each sale', 'Manager reconciles sales and reorders stock'],
  ...
  pharmacy_retail: {
    ...
    flows: ['Pharmacist receives medicine shipment from supplier', 'Customer brings prescription Ã¢â‚¬â€ pharmacist verifies', 'Pharmacist dispenses medicine Ã¢â‚¬â€ cashier processes sale', 'Stock is updated after each transaction', 'Manager reorders when stock reaches minimum level'],
  ...
  generic: {
    ...
    entities: {
      // Only 3 minimal entities Ã¢â‚¬â€ everything else generated by AI
      Project: {
    ...
    flows: [
      'User login Ã¢â‚¬â€ melihat dashboard ringkasan',
      'User dapat membuat item baru Ã¢â‚¬â€ form dengan field yang tervalidasi',
      'User dapat melihat daftar item Ã¢â‚¬â€ search, filter, sort, pagination',
      'User dapat mengedit item Ã¢â‚¬â€ simpan perubahan',
      'User dapat menghapus/archive item Ã¢â‚¬â€ soft delete',
    ],
    endpoints: [
...
// Keyword Ã¢â€ â€™ domain scoring map
...
function getDomain() {
  ...
  // Phase 1: Keyword scoring Ã¢â‚¬â€ calculate match strength for ALL domains
  const scores = {};
  const maxPossible = {};
  let bestScore = 0;
  ...
  for (const [domain, terms] of Object.entries(DOMAIN_KEYWORDS)) {
    ...
    for (const term of terms) {
      ...
      if (idea.includes(term)) {
        // Longer terms = more specific Ã¢â€ â€™ higher weight
        score += term.length;
      }
    }
    scores[domain] = score;
...
function buildEntityDiagram(domainPack) {
  ...
  for (const [entityName, entityDef] of Object.entries(domainPack.entities)) {
    ...
    // Collect relations Ã¢â‚¬â€ belongsTo
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Inline Pipeline Runner (fallback if createArtifacts not yet initialized) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Step 4: Blueprint Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
async function generateBlueprint() {
  ...
  try {
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ AI Enhancement: Generate PRD with AI Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    try {
      ...
      if (state.productCategoryParent) catParts.push(state.productCategoryParent);
      if (state.productCategory) catParts.push(state.productCategory);
      const catStr = catParts.join(' Ã¢â€ â€™ ') || '-';
      ...
      if (window._aiTechRec) {
        ...
        for (var ti = 0; ti < layers.length; ti++) {
          var lr = t[layers[ti]];
          if (lr && lr.rec) techStr += '- ' + layers[ti] + ': ' + lr.rec + ' (alt: ' + (lr.alt || '-') + ') Ã¢â‚¬â€ ' + (lr.reason || '') + '\n';
        }
      ...
      const prompt = [
        ...
        '',
        '## 5. User Flows',
        'Alur lengkap per aktor Ã¢â‚¬â€ step-by-step dari awal sampai selesai.',
        '',
        '## 6. API Design',
        ...
        '',
        'Aturan:',
        '- SEMUA KONTEN SPESIFIK untuk domain ' + domain + ' Ã¢â‚¬â€ jangan generic',
      ...
      // Fallback Ã¢â‚¬â€ pipeline's createArtifacts already built the PRD
    } finally {
...
function dedupFeatures(features) {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ ENGINE FLAGS Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  const ENGINE_FLAGS = {
    domain: true,
    relationships: true,
    modules: true,
  ...
  // (Analytics functions are already defined at top level Ã¢â‚¬â€ see lines ~651)
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Project History (localStorage) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ ARTIFACT STORAGE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  let engineArtifacts = {};

  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V1 DOMAIN ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V2 RELATIONSHIP ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V5 MODULE ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V8 VALIDATION ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V10 ARCHITECTURE ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V13 SECURITY ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V16 DOCUMENTATION ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  function runDocumentationEngine(domain, relations, modules, validation, architecture, security) {
    const docs = {
      ...
      // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V3 STATE MACHINE ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
      ...
      for (const [entityName, entityDef] of Object.entries(pack.entities)) {
        ...
        // Generate transitions: each state Ã¢â€ â€™ next state (forward flow)
        const transitions = [];
      ...
      // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V7 UI FLOW ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
      ...
      const journeys = pack.actors.map(actor => {
        ...
        return {
          ...
          steps: relevantFlows.length > 0
            ? relevantFlows.slice(0, 5).map(f => ({ action: f.split('Ã¢â‚¬â€')[0]?.trim() || f, description: f }))
      ...
      // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ V4 EVENT ENGINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
      ...
      // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ CORE PIPELINE Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  function createArtifacts(engineArtifactsFromPipeline) {
    ...
    const domainHints = {
      ...
      inventory: ['User dapat menambah produk baru dengan SKU, nama, satuan, dan harga', 'Sistem harus memvalidasi SKU unik sebelum menyimpan produk', 'Stok masuk dicatat dengan nomor batch, tanggal kedaluwarsa, dan lokasi rak', 'Stok keluar divalidasi terhadap jumlah tersedia, dan gagal jika stok tidak mencukupi', 'Low stock alert muncul ketika stok di bawah batas minimum yang ditentukan', 'Validasi: jumlah stok tidak boleh negatif', 'Error message: "Stok tidak mencukupi. Tersedia: {qty}"', 'Edge case: dua admin melakukan stok keluar bersamaan Ã¢â‚¬â€ pakai optimistic lock atau queue'],
      crm: ['User dapat membuat lead baru dengan nama, perusahaan, dan nomor kontak', 'Sistem harus mencatat sumber lead (referral, website, cold call)', 'Lead dapat dipindahkan antar status pipeline (new -> contacted -> qualified -> deal)', 'Sistem mencatat timestamp setiap perubahan status lead', 'User dapat menambahkan follow-up activity (call, email, meeting) ke lead', 'Validasi: nomor telepon minimal 10 digit', 'Error message: "Nomor telepon tidak valid. Minimal 10 digit"', 'Edge case: lead dihapus oleh admin saat sales sedang mengedit Ã¢â‚¬â€ tampilkan notifikasi'],
      habit: ['User dapat membuat habit baru dengan nama, frekuensi (harian/mingguan), dan target', 'Daily check-in selesai dengan satu tap Ã¢â‚¬â€ mencatat timestamp dan streak otomatis', 'Streak terputus jika user melewatkan 1 hari untuk habit harian', 'Progress calendar menunjukkan hari hijau (selesai), abu-abu (terlewat), dan streak saat ini', 'Sistem mengirim insight mingguan berdasarkan pola check-in 7 hari terakhir', 'Validasi: nama habit tidak boleh kosong, maks 100 karakter', 'Error message: "Nama habit wajib diisi"', 'Edge case: user check-in jam 23:59 dan 00:01 Ã¢â‚¬â€ treat berdasarkan timezone user'],
      booking: ['User dapat melihat slot tersedia berdasarkan layanan dan tanggal yang dipilih', 'Booking memblokir slot selama 15 menit (pending) sebelum dikonfirmasi', 'Sistem harus mencegah double-booking pada slot yang sama', 'User dapat reschedule booking maksimal 24 jam sebelum jadwal', 'Sistem mengirim reminder 1 jam sebelum jadwal via notifikasi', 'Validasi: waktu booking harus di masa depan, tidak boleh di masa lalu', 'Error message: "Slot sudah di booking. Pilih waktu lain"', 'Edge case: admin menghapus layanan yang memiliki booking aktif Ã¢â‚¬â€ arsipkan booking'],
      finance: ['User dapat mencatat pemasukan dan pengeluaran dengan kategori, nominal, dan tanggal', 'Sistem harus menghitung saldo otomatis berdasarkan semua transaksi', 'Ringkasan cashflow bulanan: total pemasukan, total pengeluaran, selisih', 'User dapat membuat invoice sederhana dengan generate nomor otomatis', 'Export laporan ke CSV dengan rentang tanggal yang dipilih', 'Validasi: nominal harus lebih dari 0', 'Error message: "Nominal harus lebih dari 0"', 'Edge case: transaksi di masa depan (pre-order) Ã¢â‚¬â€ treat sebagai pending'],
      commerce: ['User dapat melihat katalog produk dengan gambar, harga, dan stok', 'User dapat menambahkan produk ke cart dan mengubah quantity', 'Checkout memvalidasi stok dan menghitung total otomatis (termasuk ongkir jika ada)', 'Sistem mengurangi stok setelah pembayaran berhasil dikonfirmasi', 'User dapat melihat status order: pending, diproses, dikirim, selesai', 'Validasi: quantity tidak boleh melebihi stok tersedia', 'Error message: "Stok tidak mencukupi untuk {produk}"', 'Edge case: 2 user checkout produk yang sama di saat bersamaan Ã¢â‚¬â€ pesanan pertama yang di approve'],
      restaurant: ['Customer order menu Ã¢â€ â€™ waiter mengambil order Ã¢â€ â€™ kitchen prepares Ã¢â€ â€™ serves Ã¢â€ â€™ payment', 'Waiter harus mencatat order dengan benar dan mengkonfirmasi ke customer', 'Kitchen menerima order dan mempersiapkan sesuai request', 'Served ke meja customer setelah selesai dimasak', 'Payment dilakukan setelah customer selesai makan', 'Validasi: menu item harus tersedia', 'Error message: "Menu tidak tersedia"', 'Edge case: customer cancel setelah cooking starts Ã¢â‚¬â€ full price masih dikenakan'],
      cafe: ['Customer order di counter Ã¢â€ â€™ barista membuat minuman Ã¢â€ â€™ customer pick up Ã¢â€ â€™ payment', 'Barista harus mengkonfirmasi order sebelum memulai proses', 'Customer mengambil minuman di pick-up area setelah notifikasi', 'Payment dilakukan saat order', 'Sistem mencatat antrian order untuk pick-up', 'Validasi: minimal order Rp 5,000', 'Error message: "Minimal order Rp 5.000"', 'Edge case: customer claims wrong order Ã¢â‚¬â€ verifikasi receipt'],
      bakery: ['Baker bakes in batches Ã¢â€ â€™ display produk Ã¢â€ â€™ customer buys Ã¢â€ â€™ payment', 'Produk ditampilkan di display case dengan harga dan label', 'Customer memilih produk dan melakukan pembayaran', 'Sistem mengurangi stock setelah payment sukses', 'Stock opname dilakukan setiap akhir hari', 'Validasi: stock harus > 0', 'Error message: "Produk habis"', 'Edge case: unsold items di akhir hari Ã¢â‚¬â€ diskon atau donasi'],
      catering: ['Customer memilih package Ã¢â€ â€™ confirm menu Ã¢â€ â€™ cook on schedule Ã¢â€ â€™ deliver Ã¢â€ â€™ payment', 'Customer harus memilih package menu yang tersedia', 'Konfirmasi menu dilakukan H-1 sebelum event', 'Masakan dipersiapkan sesuai jadwal delivery', 'Delivery dilakukan ke alamat customer', 'Validasi: order harus 24h sebelum event', 'Error message: "Minimal order H-1"', 'Edge case: delivery address salah Ã¢â‚¬â€ kontak customer segera'],
      hotel: ['Guest reserves room Ã¢â€ â€™ check-in Ã¢â€ â€™ stay Ã¢â€ â€™ housekeeping Ã¢â€ â€™ check-out Ã¢â€ â€™ payment', 'Reservasi mengunci kamar untuk tanggal tertentu', 'Check-in memverifikasi identitas guest', 'Housekeeping membersihkan kamar setiap hari', 'Check-out menghitung total biaya menginap', 'Validasi: check-out harus setelah check-in', 'Error message: "Check-out harus setelah check-in"', 'Edge case: guest extends stay Ã¢â‚¬â€ cek availability dan adjust rate'],
      salon: ['Customer books service Ã¢â€ â€™ arrives Ã¢â€ â€™ stylist serves Ã¢â€ â€™ payment Ã¢â€ â€™ review', 'Booking memblokir slot waktu stylist', 'Customer check-in saat tiba di salon', 'Stylist memberikan layanan sesuai booking', 'Review diberikan setelah service selesai', 'Validasi: appointment harus di masa future', 'Error message: "Slot tidak tersedia"', 'Edge case: stylist calls in sick Ã¢â‚¬â€ reassign atau reschedule'],
      barbershop: ['Customer walks in atau books Ã¢â€ â€™ queue Ã¢â€ â€™ barber serves Ã¢â€ â€™ payment', 'Customer check-in untuk masuk antrian', 'Barber memanggil customer berdasarkan antrian', 'Layanan diberikan sesuai permintaan customer', 'Payment dilakukan setelah selesai', 'Validasi: customer harus check in first', 'Error message: "Antrian penuh"', 'Edge case: customer wants service beyond menu Ã¢â‚¬â€ inform price first'],
      workshop: ['Customer brings vehicle Ã¢â€ â€™ diagnose Ã¢â€ â€™ estimate Ã¢â€ â€™ repair Ã¢â€ â€™ payment', 'Teknisi mendiagnosa masalah kendaraan', 'Estimasi biaya diberikan sebelum repair', 'Repair dilakukan setelah disetujui customer', 'Payment dilakukan setelah repair selesai', 'Validasi: spare part harus in stock', 'Error message: "Spare part tidak tersedia"', 'Edge case: repair takes longer dari estimasi Ã¢â‚¬â€ notifikasi customer'],
      cleaning_service: ['Customer orders Ã¢â€ â€™ assign cleaner Ã¢â€ â€™ clean Ã¢â€ â€™ verify Ã¢â€ â€™ payment', 'Customer memilih paket cleaning dan jadwal', 'Cleaner ditugaskan berdasarkan area dan availability', 'Cleaning dilakukan sesuai standar operasional', 'Customer verifikasi hasil cleaning', 'Validasi: address harus di service area', 'Error message: "Area belum terjangkau"', 'Edge case: cleaner tidak datang Ã¢â‚¬â€ reassign atau refund'],
      field_service: ['Customer requests Ã¢â€ â€™ dispatcher assigns Ã¢â€ â€™ technician arrives Ã¢â€ â€™ service Ã¢â€ â€™ complete', 'Dispatcher menugaskan technician berdasarkan skill dan lokasi', 'Technician melakukan service di lokasi customer', 'Service completion dicatat di sistem', 'Customer sign off setelah service selesai', 'Validasi: technician harus certified', 'Error message: "Teknisi tidak tersedia"', 'Edge case: parts needed tapi tidak di inventory Ã¢â‚¬â€ order first'],
      accounting: ['Accountant records transactions Ã¢â€ â€™ journal Ã¢â€ â€™ ledger Ã¢â€ â€™ trial balance Ã¢â€ â€™ report', 'Setiap transaksi dicatat di jurnal umum', 'Posting ke ledger sesuai akun yang tepat', 'Trial balance dibuat untuk verifikasi keseimbangan', 'Laporan keuangan dihasilkan secara periodik', 'Validasi: debit = credit', 'Error message: "Debit dan credit tidak balance"', 'Edge case: previous period needs correction Ã¢â‚¬â€ gunakan reversing entry'],
      invoicing: ['Create invoice Ã¢â€ â€™ send to customer Ã¢â€ â€™ remind if overdue Ã¢â€ â€™ receive payment Ã¢â€ â€™ reconcile', 'Invoice dibuat dengan nomor unik dan detail transaksi', 'Invoice dikirim ke customer via email', 'Reminder otomatis dikirim jika overdue', 'Payment diterima dan direkonsiliasi', 'Validasi: invoice amount > 0', 'Error message: "Amount invoice harus lebih dari 0"', 'Edge case: customer disputes invoice Ã¢â‚¬â€ flag for review'],
      expense: ['Employee submits Ã¢â€ â€™ manager approves Ã¢â€ â€™ finance reimburses Ã¢â€ â€™ categorize Ã¢â€ â€™ report', 'Employee mengajukan expense report dengan bukti', 'Manager mereview dan approve/reject', 'Finance melakukan reimbursment ke employee', 'Expense dikategorikan untuk laporan', 'Validasi: receipt required untuk > 100k', 'Error message: "Bukti transaksi wajib untuk nominal di atas Rp 100.000"', 'Edge case: expense exceeds department budget Ã¢â‚¬â€ need director approval'],
      payroll: ['Collect attendance Ã¢â€ â€™ calculate salary Ã¢â€ â€™ deductions Ã¢â€ â€™ approve Ã¢â€ â€™ disburse Ã¢â€ â€™ report', 'Attendance dikumpulkan untuk periode tertentu', 'Salary dihitung berdasarkan kehadiran dan komponen gaji', 'Deductions dipotong sesuai ketentuan', 'Payroll disetujui dan didisburse', 'Validasi: salary harus > minimum wage', 'Error message: "Gaji tidak boleh di bawah UMR"', 'Edge case: employee joins mid-month Ã¢â‚¬â€ prorate salary'],
      budgeting: ['Department proposes Ã¢â€ â€™ finance reviews Ã¢â€ â€™ approved Ã¢â€ â€™ track spending Ã¢â€ â€™ adjust Ã¢â€ â€™ report', 'Department mengajukan proposal anggaran', 'Finance mereview dan menyetujui', 'Pengeluaran di-track terhadap anggaran', 'Adjustment dilakukan jika diperlukan', 'Validasi: total budget harus match company target', 'Error message: "Total melebihi anggaran perusahaan"', 'Edge case: emergency spending Ã¢â‚¬â€ gunakan contingency fund'],
      attendance: ['Employee checks in Ã¢â€ â€™ tracks hours Ã¢â€ â€™ approves leave Ã¢â€ â€™ calculate overtime Ã¢â€ â€™ report', 'Check-in mencatat waktu kedatangan', 'Check-out mencatat waktu pulang', 'Leave diapprove oleh manager', 'Overtime dihitung otomatis', 'Validasi: check-in time harus recorded', 'Error message: "Absensi gagal. Hubungi HR"', 'Edge case: employee lupa check-out Ã¢â‚¬â€ auto-calculate atau manual input'],
      recruitment: ['HR posts job Ã¢â€ â€™ candidates apply Ã¢â€ â€™ screen Ã¢â€ â€™ interview Ã¢â€ â€™ offer Ã¢â€ â€™ hire', 'Job vacancy dipublikasikan di portal', 'Kandidat mengirimkan aplikasi', 'HR melakukan screening awal', 'Interview dijadwalkan untuk kandidat terpilih', 'Offer diberikan ke kandidat terbaik', 'Validasi: candidate harus meet minimum qualifications', 'Error message: "Kandidat tidak memenuhi kualifikasi"', 'Edge case: offer rejected Ã¢â‚¬â€ move ke second candidate'],
      employee_management: ['HR creates employee record Ã¢â€ â€™ assign department Ã¢â€ â€™ manage documents Ã¢â€ â€™ transfer Ã¢â€ â€™ offboard', 'Employee record dibuat dengan data lengkap', 'Employee ditugaskan ke department', 'Dokumen karyawan dikelola di sistem', 'Transfer atau promosi dicatat', 'Offboarding dilakukan saat resign', 'Validasi: employee ID harus unique', 'Error message: "ID karyawan sudah terdaftar"', 'Edge case: employee resigns tanpa notice Ã¢â‚¬â€ process offboarding ASAP'],
      performance_management: ['Set goals Ã¢â€ â€™ mid-year review Ã¢â€ â€™ annual review Ã¢â€ â€™ rating Ã¢â€ â€™ feedback Ã¢â€ â€™ improvement plan', 'Goals ditetapkan di awal periode', 'Mid-year review mengevaluasi progress', 'Annual review memberikan rating final', 'Feedback diberikan ke employee', 'Improvement plan dibuat jika perlu', 'Validasi: goals harus SMART', 'Error message: "Goal harus spesifik dan terukur"', 'Edge case: employee disagree dengan rating Ã¢â‚¬â€ escalation ke HR'],
      fleet: ['Fleet manager assigns vehicle Ã¢â€ â€™ driver takes trip Ã¢â€ â€™ track fuel Ã¢â€ â€™ maintenance Ã¢â€ â€™ report', 'Vehicle ditugaskan ke driver untuk trip', 'Driver mencatat trip details', 'Fuel consumption di-track', 'Maintenance dijadwalkan secara periodik', 'Validasi: driver harus memiliki valid license', 'Error message: "Driver tidak memiliki SIM valid"', 'Edge case: vehicle breaks down during trip Ã¢â‚¬â€ dispatch backup'],
      courier: ['Sender creates shipment Ã¢â€ â€™ courier picks up Ã¢â€ â€™ sort Ã¢â€ â€™ transit Ã¢â€ â€™ deliver Ã¢â€ â€™ confirm', 'Shipment dibuat dengan detail pengirim dan penerima', 'Courier pick up paket dari pengirim', 'Paket di-sort di hub', 'Transit ke tujuan', 'Paket di-deliver dan dikonfirmasi', 'Validasi: address harus valid', 'Error message: "Alamat tidak valid"', 'Edge case: recipient tidak di rumah Ã¢â‚¬â€ reschedule delivery'],
      trucking: ['Shipper books load Ã¢â€ â€™ dispatcher assigns truck Ã¢â€ â€™ driver loads Ã¢â€ â€™ transit Ã¢â€ â€™ deliver Ã¢â€ â€™ confirm', 'Load booking dibuat dengan detail barang', 'Dispatcher menugaskan truck dan driver', 'Driver melakukan loading barang', 'Transit ke tujuan pengiriman', 'Delivery dikonfirmasi oleh penerima', 'Validasi: load weight tidak boleh exceed capacity', 'Error message: "Berat melebihi kapasitas kendaraan"', 'Edge case: road closure Ã¢â‚¬â€ reroute dan notifikasi customer'],
      digital_product: ['Creator creates product Ã¢â€ â€™ list in store Ã¢â€ â€™ customer buys Ã¢â€ â€™ delivers automatically Ã¢â€ â€™ support', 'Digital product dibuat dengan file dan deskripsi', 'Product di-list di store', 'Customer membeli dan pembayaran diverifikasi', 'Product di-deliver otomatis ke customer', 'Support diberikan jika ada masalah', 'Validasi: file harus di bawah 2GB', 'Error message: "File terlalu besar. Maksimal 2GB"', 'Edge case: download link expires Ã¢â‚¬â€ allow regeneration'],
      membership: ['User registers Ã¢â€ â€™ chooses tier Ã¢â€ â€™ pays Ã¢â€ â€™ accesses benefits Ã¢â€ â€™ renews or downgrades', 'User mendaftar dengan data diri', 'User memilih membership tier', 'Payment dikonfirmasi sebelum akses diberikan', 'User mengakses benefits sesuai tier', 'Membership bisa diperpanjang atau di-downgrade', 'Validasi: payment harus confirmed sebelum access', 'Error message: "Pembayaran belum dikonfirmasi"', 'Edge case: payment fails tapi user sudah granted access Ã¢â‚¬â€ revoke until resolved'],
      course_platform: ['Instructor creates course Ã¢â€ â€™ publish Ã¢â€ â€™ student enrolls Ã¢â€ â€™ learns Ã¢â€ â€™ takes quiz Ã¢â€ â€™ certificate', 'Course dibuat dengan materi dan quiz', 'Course di-publish setelah review', 'Student enroll ke course', 'Student mengakses lesson secara berurutan', 'Quiz diambil untuk evaluasi', 'Validasi: max students tidak boleh exceeded', 'Error message: "Kelas penuh"', 'Edge case: student wants refund setelah 50% progress Ã¢â‚¬â€ pro-rated refund'],
      farm: ['Farmer plants crop Ã¢â€ â€™ maintains Ã¢â€ â€™ harvests Ã¢â€ â€™ stores Ã¢â€ â€™ sells', 'Petani menanam sesuai musim dan jenis tanaman', 'Perawatan dilakukan secara rutin', 'Panen dilakukan saat tanaman siap', 'Hasil panen disimpan di gudang', 'Hasil panen dijual ke buyer', 'Validasi: planting season harus valid', 'Error message: "Bukan musim tanam yang tepat"', 'Edge case: crop failure karena weather Ã¢â‚¬â€ insurance claim process'],
      livestock: ['Farmer buys animal Ã¢â€ â€™ feeds Ã¢â€ â€™ health check Ã¢â€ â€™ breeds Ã¢â€ â€™ sells', 'Hewan dibeli dari supplier', 'Pakan diberikan sesuai jadwal', 'Health check dilakukan oleh dokter hewan', 'Pembibitan dilakukan untuk reproduksi', 'Hewan dijual saat siap', 'Validasi: animal harus vaccinated', 'Error message: "Hewan belum divaksinasi"', 'Edge case: disease outbreak Ã¢â‚¬â€ quarantine dan report'],
      poultry: ['Farmer gets chicks Ã¢â€ â€™ raises Ã¢â€ â€™ feeds Ã¢â€ â€™ vaccinates Ã¢â€ â€™ sells', 'DOC (Day Old Chick) dibeli dari hatchery', 'Pemeliharaan dilakukan di kandang', 'Pakan dan air diberikan secara teratur', 'Vaksinasi sesuai jadwal', 'Ayam dijual saat mencapai berat target', 'Validasi: coop temperature harus controlled', 'Error message: "Suhu kandang tidak sesuai"', 'Edge case: high mortality rate Ã¢â‚¬â€ investigate cause'],
      bengkel: ['Customer brings vehicle Ã¢â€ â€™ mechanic diagnoses Ã¢â€ â€™ give estimate Ã¢â€ â€™ repair Ã¢â€ â€™ payment', 'Customer datang dengan kendaraan bermasalah', 'Mekanik mendiagnosa kerusakan', 'Estimasi biaya dan waktu diberikan', 'Repair dilakukan setelah disetujui', 'Payment dilakukan setelah selesai', 'Validasi: spare part harus original atau approved alternative', 'Error message: "Spare part tidak tersedia"', 'Edge case: repair cost exceeds estimate Ã¢â‚¬â€ get customer approval first'],
      car_rental: ['Customer books car Ã¢â€ â€™ pick up Ã¢â€ â€™ rent period Ã¢â€ â€™ return Ã¢â€ â€™ inspection Ã¢â€ â€™ payment', 'Customer booking mobil dengan durasi sewa', 'Pick up mobil di lokasi yang disepakati', 'Mobil digunakan selama periode sewa', 'Mobil dikembalikan dan diinspeksi', 'Payment dihitung berdasarkan durasi dan kondisi', 'Validasi: driver license harus valid', 'Error message: "SIM tidak valid"', 'Edge case: car returned damaged Ã¢â‚¬â€ charge sesuai policy'],
      dealer: ['Customer browses Ã¢â€ â€™ test drive Ã¢â€ â€™ negotiate Ã¢â€ â€™ financing Ã¢â€ â€™ deliver', 'Customer melihat unit yang tersedia', 'Test drive dijadwalkan', 'Negosiasi harga dilakukan', 'Financing diajukan jika perlu', 'Unit di-deliver setelah deal', 'Validasi: customer harus pass financing check', 'Error message: "Pembiayaan tidak disetujui"', 'Edge case: customer ingin return dalam 3 hari Ã¢â‚¬â€ apply return policy'],
      contractor: ['Client signs contract Ã¢â€ â€™ plan Ã¢â€ â€™ execute Ã¢â€ â€™ monitor Ã¢â€ â€™ complete Ã¢â€ â€™ handover', 'Kontrak ditandatangani dengan scope kerja jelas', 'Perencanaan proyek dibuat', 'Eksekusi dilakukan sesuai jadwal', 'Progress di-monitor secara berkala', 'Handover dilakukan setelah selesai', 'Validasi: project budget harus approved', 'Error message: "Anggaran proyek belum disetujui"', 'Edge case: weather delays project Ã¢â‚¬â€ adjust timeline'],
      maintenance: ['Tenant reports issue Ã¢â€ â€™ assign technician Ã¢â€ â€™ schedule Ã¢â€ â€™ fix Ã¢â€ â€™ verify Ã¢â€ â€™ close', 'Tenant melaporkan issue maintenance', 'Technician ditugaskan berdasarkan issue', 'Jadwal kunjungan ditentukan', 'Perbaikan dilakukan di lokasi', 'Tenant memverifikasi perbaikan selesai', 'Validasi: priority harus set', 'Error message: "Priority wajib diisi"', 'Edge case: issue not fixed setelah first visit Ã¢â‚¬â€ reschedule'],
      event_management: ['Organizer creates event Ã¢â€ â€™ promote Ã¢â€ â€™ tickets sell Ã¢â€ â€™ check-in Ã¢â€ â€™ execute Ã¢â€ â€™ follow-up', 'Event dibuat dengan detail tanggal dan venue', 'Promosi dilakukan melalui berbagai channel', 'Tiket dijual online', 'Check-in dilakukan saat event', 'Event dilaksanakan sesuai rencana', 'Validasi: event date harus di future', 'Error message: "Tanggal event harus di masa depan"', 'Edge case: event cancelled Ã¢â‚¬â€ auto-refund semua tickets'],
      forum: ['User creates thread Ã¢â€ â€™ others discuss Ã¢â€ â€™ moderator reviews Ã¢â€ â€™ votes Ã¢â€ â€™ archive', 'Thread dibuat dengan judul dan konten', 'User lain berdiskusi di thread', 'Moderator mereview konten', 'Voting dilakukan untuk kualitas thread', 'Thread di-archive jika sudah tidak aktif', 'Validasi: title harus unique', 'Error message: "Judul thread sudah ada"', 'Edge case: spam detected Ã¢â‚¬â€ auto-hide dan notifikasi moderator'],
      membership_community: ['User applies Ã¢â€ â€™ approved Ã¢â€ â€™ pays dues Ã¢â€ â€™ participates Ã¢â€ â€™ renews', 'User mengajukan aplikasi membership', 'Admin meng-approve aplikasi', 'User membayar iuran', 'User berpartisipasi di komunitas', 'Membership diperpanjang secara periodik', 'Validasi: email harus unique', 'Error message: "Email sudah terdaftar"', 'Edge case: member violates code of conduct Ã¢â‚¬â€ warn, suspend, atau remove'],
      photography: ['Client books package Ã¢â€ â€™ photographer shoots Ã¢â€ â€™ edits Ã¢â€ â€™ delivers Ã¢â€ â€™ reviews', 'Client booking paket photography', 'Photographer melakukan sesi foto', 'Foto diedit dan dipilih', 'Hasil foto di-deliver ke client', 'Client memberikan review', 'Validasi: booking harus confirmed dengan deposit', 'Error message: "DP wajib dibayar untuk konfirmasi booking"', 'Edge case: client unhappy dengan results Ã¢â‚¬â€ offer reshoot atau partial refund'],
      veterinary: ['Pet owner registers pet Ã¢â€ â€™ appointment Ã¢â€ â€™ vet examines Ã¢â€ â€™ diagnosis Ã¢â€ â€™ treatment Ã¢â€ â€™ follow-up', 'Pet didaftarkan dengan data lengkap', 'Appointment dibuat dengan jadwal', 'Vet memeriksa kondisi hewan', 'Diagnosis diberikan ke owner', 'Treatment dilakukan sesuai diagnosis', 'Validasi: pet harus registered', 'Error message: "Hewan belum terdaftar"', 'Edge case: emergency case Ã¢â‚¬â€ prioritize over scheduled appointments'],
      gym: ['Member registers Ã¢â€ â€™ chooses plan Ã¢â€ â€™ pays Ã¢â€ â€™ attends classes Ã¢â€ â€™ tracks progress Ã¢â€ â€™ renews', 'Member mendaftar dengan data diri', 'Member memilih plan membership', 'Payment dilakukan untuk aktivasi', 'Member attend kelas sesuai jadwal', 'Progress di-track di sistem', 'Validasi: membership harus active', 'Error message: "Membership tidak aktif"', 'Edge case: class is full Ã¢â‚¬â€ put on waitlist'],
      coworking: ['Member browses spaces Ã¢â€ â€™ books Ã¢â€ â€™ checks in Ã¢â€ â€™ uses Ã¢â€ â€™ checks out Ã¢â€ â€™ billed', 'Member melihat available spaces', 'Booking dibuat dengan durasi', 'Check-in saat tiba di lokasi', 'Member menggunakan fasilitas', 'Check-out dan billing dihitung', 'Validasi: booking harus minimal 1 jam', 'Error message: "Minimal booking 1 jam"', 'Edge case: member stays overtime Ã¢â‚¬â€ charge extra hour'],
      generic: ['User dapat membuat item baru dengan form yang tervalidasi', 'Sistem menampilkan daftar item dengan search, filter, sort, dan pagination', 'User dapat mengedit item yang sudah ada dan menyimpan perubahannya', 'User dapat mengarsipkan item (soft delete) Ã¢â‚¬â€ tidak hilang dari database', 'Dashboard menampilkan ringkasan: total item, item aktif, item terbaru', 'Validasi: field nama/email wajib diisi, field angka harus numerik', 'Error message: "Field {field} wajib diisi"', 'Edge case: user membuka 2 tab dan mengedit item yang sama Ã¢â‚¬â€ last write wins dengan konfirmasi'],
      laundry: ['Staff dapat membuat order laundry baru dengan mencatat customer, item, dan berat', 'Sistem menghitung total harga otomatis berdasarkan item dan berat', 'Status laundry diupdate setiap tahap: washing Ã¢â€ â€™ drying Ã¢â€ â€™ ironing Ã¢â€ â€™ packing', 'Customer dapat melihat status laundry via link tracking', 'Payment dicatat setelah customer membayar Ã¢â‚¬â€ status order berubah', 'Validasi: berat laundry harus lebih dari 0', 'Error message: "Berat laundry minimal 0.5 kg"', 'Edge case: customer membatalkan order saat laundry sudah diproses Ã¢â‚¬â€ konfirmasi owner'],
      pos: ['Cashier dapat scan barcode produk untuk menambahkan ke transaksi', 'Sistem menampilkan nama produk dan harga otomatis setelah scan', 'Sistem menghitung total, diskon, pajak, dan kembalian dengan benar', 'Stok otomatis berkurang setelah transaksi selesai', 'Cashier dapat tutup kasir di akhir shift Ã¢â‚¬â€ sistem hitung total penjualan', 'Validasi: nominal bayar tidak boleh kurang dari total belanja', 'Error message: "Uang tidak mencukupi. Kurang Rp {amount}"', 'Edge case: barcode tidak terbaca Ã¢â‚¬â€ cashier bisa cari produk manual via search'],
      erp: ['Admin dapat mendaftarkan department dan employee baru dengan data lengkap', 'Employee dapat mengajukan leave dengan tipe dan tanggal yang jelas', 'Manager dapat approve/reject leave Ã¢â‚¬â€ sistem update status otomatis', 'Finance dapat mengatur budget per department dan tracking pengeluaran', 'Manager dapat assign task ke employee dengan deadline dan prioritas', 'Validasi: leave date end tidak boleh sebelum start', 'Error message: "Tanggal akhir cuti tidak boleh sebelum tanggal mulai"', 'Edge case: employee mengajukan leave di hari yang sama dengan employee lain Ã¢â‚¬â€ overlap detection'],
      manufacturing: ['Production Manager dapat membuat production order dengan produk dan quantity', 'Operator memulai produksi Ã¢â‚¬â€ sistem catat start time dan kurangi material', 'QC Staff menginspeksi hasil produksi Ã¢â‚¬â€ mencatat pass/fail quantity', 'Produk jadi masuk gudang Ã¢â‚¬â€ stok ditambahkan otomatis', 'Dashboard menampilkan order aktif, produksi hari ini, dan reject rate', 'Validasi: quantity harus lebih dari 0', 'Error message: "Quantity produksi harus lebih dari 0"', 'Edge case: material tidak mencukupi saat produksi dimulai Ã¢â‚¬â€ sistem beri alert dan hold order'],
      healthcare: ['Receptionist dapat membuat appointment untuk pasien dengan memilih dokter dan waktu', 'Pasien check-in Ã¢â‚¬â€ sistem update status dan notifikasi dokter', 'Dokter mencatat diagnosis dan resep Ã¢â‚¬â€ tersimpan di medical record', 'Pasien bayar di kasir Ã¢â‚¬â€ invoice digenerate otomatis', 'Dashboard menampilkan pasien hari ini, pendapatan, jadwal dokter', 'Validasi: appointment time harus di masa depan', 'Error message: "Slot dokter sudah penuh. Pilih waktu lain"', 'Edge case: pasien no-show Ã¢â‚¬â€ sistem tandai dan biaya consult tetap dikenakan'],
      education: ['Admin dapat membuat course dengan lesson, materi, dan harga', 'Student mendaftar course Ã¢â‚¬â€ enrollment aktif dan progress mulai 0%', 'Student mengakses lesson Ã¢â‚¬â€ sistem catat progress per lesson', 'Student submit assignment Ã¢â‚¬â€ teacher dapat memberi score', 'Dashboard teacher menampilkan student progress, nilai, completion rate', 'Validasi: maxStudents tidak boleh kurang dari jumlah enrolled', 'Error message: "Kelas sudah penuh. Maksimal {max} student"', 'Edge case: student mengumpulkan assignment setelah deadline Ã¢â‚¬â€ teacher bisa tetap menilai dengan penalti'],
      property: ['Owner/Agent dapat mendaftarkan properti dengan tipe, alamat, dan harga', 'Tenant dapat melihat properti dan unit yang tersedia', 'Tenant memilih unit Ã¢â‚¬â€ lease agreement dibuat dengan durasi sewa', 'Tenant bayar sewa bulanan Ã¢â‚¬â€ sistem catat payment dan update status', 'Maintenance request dibuat Ã¢â‚¬â€ owner assign dan track progress', 'Validasi: endDate lease harus setelah startDate', 'Error message: "Tanggal akhir sewa harus setelah tanggal mulai"', 'Edge case: tenant telat bayar sewa Ã¢â‚¬â€ sistem kirim reminder otomatis dan hitung denda'],
      pharmacy: ['Pharmacist dapat menambah obat baru dengan SKU, kategori, harga, dan stok awal', 'Sistem harus memvalidasi SKU unik sebelum menyimpan obat', 'Resep dari dokter divalidasi oleh apoteker sebelum dispensing', 'Stok otomatis berkurang saat obat dijual atau didispense', 'Low stock alert muncul ketika stok di bawah batas minimum', 'Validasi: dosis dan kuantitas resep harus masuk akal (tidak melebihi maksimum)', 'Error message: \"Stok tidak mencukupi. Tersedia: {qty}\"', 'Edge case: obat mendekati expired Ã¢â‚¬â€ beri peringatan saat dispensing'],
      laboratory: ['LabTech dapat menambah test baru dengan kategori, harga, dan persiapan', 'Sample harus dilabel dengan benar dan ditracking dari koleksi hingga hasil', 'Test dapat memiliki beberapa parameter hasil dengan reference range', 'Hasil abnormal otomatis ditandai untuk review dokter', 'Sistem mencatat timestamp setiap perubahan status sample', 'Validasi: sample harus sampai di lab sebelum hasil bisa dimasukkan', 'Error message: \"Sample tidak ditemukan\"', 'Edge case: sample rusak atau hilang Ã¢â‚¬â€ catat sebagai reject dan minta sample baru'],
      telemedicine: ['Dokter dapat mengatur jadwal ketersediaan untuk konsultasi online', 'Konsultasi dimulai tepat waktu sesuai jadwal booking', 'Sistem mencatat seluruh sesi konsultasi termasuk resep', 'Resep dikirim ke pasien secara digital setelah konsultasi', 'Riwayat medis pasien diperbarui setelah setiap konsultasi', 'Validasi: durasi konsultasi tidak boleh melebihi slot yang ditentukan', 'Error message: \"Dokter sedang tidak tersedia. Pilih jadwal lain\"', 'Edge case: koneksi terputus saat konsultasi Ã¢â‚¬â€ fitur reconnection otomatis'],
      tutoring: ['Tutor dapat menentukan tarif per jam dan jadwal mengajar', 'Sesi tutoring terjadwal mengikat antara tutor dan siswa', 'Sistem mencatat kehadiran dan durasi sesi', 'Pekerjaan rumah diberikan dan dinilai oleh tutor', 'Laporan perkembangan siswa dikirim ke orang tua secara periodik', 'Validasi: jadwal tutor tidak boleh bentrok', 'Error message: \"Jadwal tutor sudah terisi. Pilih waktu lain\"', 'Edge case: tutor tiba-tiba sakit Ã¢â‚¬â€ cari tutor pengganti atau reschedule'],
      bootcamp: ['Program bootcamp memiliki kurikulum terstruktur dengan modul berurutan', 'Mentor dapat membuat assignment dengan deadline dan kriteria penilaian', 'Siswa mengumpulkan tugas dan mentor memberikan skor serta feedback', 'Progress siswa terlihat di dashboard untuk monitoring', 'Sertifikat diberikan setelah semua modul selesai', 'Validasi: submission harus sebelum deadline', 'Error message: \"Pengumpulan sudah melewati deadline\"', 'Edge case: siswa tertinggal modul karena alasan khusus Ã¢â‚¬â€ beri perpanjangan waktu'],
      school_management: ['Admin dapat mendaftarkan siswa baru dan assign ke kelas', 'Absensi dicatat setiap hari untuk setiap siswa', 'Nilai dimasukkan per mata pelajaran per semester', 'Jadwal pelajaran diatur per kelas per hari', 'Rapor dihasilkan dari gabungan nilai dan absensi', 'Validasi: nomor induk siswa harus unik', 'Error message: \"NISN sudah terdaftar\"', 'Edge case: siswa pindah kelas di tengah semester Ã¢â‚¬â€ data tetap lengkap'],
      lms: ['Instructor dapat membuat course dengan lesson dan quiz', 'Learner dapat mengakses materi sesuai urutan yang ditentukan', 'Quiz menilai pemahaman dengan passing score tertentu', 'Progress disimpan per user per course dan lesson', 'Sertifikat diterbitkan setelah course selesai dengan passing grade', 'Validasi: learner harus menyelesaikan lesson sebelum quiz', 'Error message: \"Selesaikan semua lesson sebelum mengakses quiz\"', 'Edge case: learner ingin mengulang quiz Ã¢â‚¬â€ gunakan attempt terbaik'],
      personal_finance: ['User dapat mencatat transaksi pemasukan dan pengeluaran setiap hari', 'Transaksi otomatis dikategorikan berdasarkan jenis dan deskripsi', 'Budget ditentukan per kategori per periode (bulanan)', 'Sistem menampilkan perbandingan realisasi vs budget', 'Report bulanan menunjukkan ringkasan keuangan', 'Validasi: nominal transaksi harus lebih dari 0', 'Error message: \"Budget telah melebihi batas untuk kategori ini\"', 'Edge case: transaksi berulang bulanan Ã¢â‚¬â€ set recurrence pattern'],
      cooperative: ['Anggota baru mendaftar dan mulai menabung secara teratur', 'Pinjaman diajukan dengan jumlah, bunga, dan tenor tertentu', 'Setiap pinjaman diangsur secara bulanan dengan jadwal tetap', 'Sistem menghitung otomatis sisa pinjaman dan bunga berjalan', 'SHU/Dividen dibagikan tahunan berdasarkan simpanan dan transaksi', 'Validasi: jumlah pinjaman tidak boleh melebihi saldo simpanan * 3', 'Error message: \"Pinjaman melebihi batas maksimal\"', 'Edge case: anggota menunggak angsuran Ã¢â‚¬â€ sistem hitung denda otomatis'],
      insurance: ['Client mengajukan permohonan polis dengan data lengkap', 'Agent melakukan underwriting dan menentukan risiko', 'Premi dibayar sesuai jadwal Ã¢â‚¬â€ status polis menjadi aktif', 'Klaim diajukan dengan dokumen pendukung lengkap', 'Adjuster menyelidiki validitas klaim sebelum settlement', 'Validasi: premi harus dibayar sebelum polis bisa aktif', 'Error message: \"Dokumen klaim tidak lengkap\"', 'Edge case: klaim diajukan setelah polis expired Ã¢â‚¬â€ ditolak otomatis'],
      warehouse: ['Staff menerima barang dan mencatat di sistem dengan quantity dan bin', 'Barang disimpan di bin location yang sesuai dengan kategorinya', 'Picking barang dilakukan berdasarkan FIFO atau FEFO', 'Packing dan shipping dicatat dengan nomor referensi', 'Manager dapat melihat qty stok per produk dan per bin', 'Validasi: bin harus punya sisa kapasitas sebelum menempatkan barang', 'Error message: \"Bin sudah penuh. Pilih bin lain\"', 'Edge case: 2 staff melakukan picking barang yang sama Ã¢â‚¬â€ pakai locking system'],
      cold_chain: ['Operator memuat produk yang sensitif suhu ke dalam kendaraan', 'Sensor suhu aktif selama pengiriman dan mencatat secara real-time', 'Sistem mengirim alert jika suhu melebihi threshold yang ditentukan', 'Semua log suhu tersimpan untuk audit trail', 'Pengiriman diverifikasi setelah tiba dengan cek log suhu', 'Validasi: suhu harus dalam range yang ditentukan produk', 'Error message: \"Suhu melebihi batas aman\"', 'Edge case: sensor mati di tengah perjalanan Ã¢â‚¬â€ gunakan data dari sensor cadangan'],
      freight: ['Shipper meminta quote pengiriman dengan detail berat dan tujuan', 'Carrier memberikan harga Ã¢â‚¬â€ shipper konfirmasi booking', 'Carrier pickup barang dari shipper dan catat di sistem', 'Tracking event diperbarui di setiap tahap perjalanan', 'Barang diterima dan dikonfirmasi oleh penerima', 'Validasi: berat tidak boleh melebihi kapasitas kendaraan', 'Error message: \"Berat melebihi kapasitas maksimal\"', 'Edge case: cuaca buruk menyebabkan delay Ã¢â‚¬â€ update ETA dan notifikasi'],
      homestay: ['Host dapat mendaftarkan properti dengan detail dan harga', 'Guest mencari dan booking properti untuk tanggal tertentu', 'Booking mengunci kalender untuk tanggal yang dipilih', 'Check-in dan check-out dicatat oleh host', 'Pembayaran diproses setelah check-in atau sesuai kebijakan', 'Validasi: check-out harus setelah check-in', 'Error message: \"Tanggal sudah dibooking. Pilih tanggal lain\"', 'Edge case: guest tidak datang (no-show) Ã¢â‚¬â€ deposit tidak dikembalikan'],
      villa_rental: ['Owner dapat list villa dengan harga per malam dan fasilitas', 'Guest booking villa dan bayar deposit untuk konfirmasi', 'Sisa pembayaran dilakukan saat check-in', 'Owner inspeksi villa setelah guest check-out', 'Guest memberikan review setelah masa sewa selesai', 'Validasi: deposit minimal 50% dari total sewa', 'Error message: \"Deposit belum dibayar. Booking tidak dapat dikonfirmasi\"', 'Edge case: guest merusak properti Ã¢â‚¬â€ potong dari deposit sesuai kebijakan'],
      guest_house: ['Receptionist mencatat reservasi kamar untuk guest', 'Kamar dicek ketersediaannya sebelum booking dikonfirmasi', 'Guest check-in dan diberikan akses kamar', 'Guest dapat request layanan tambahan seperti laundry', 'Check-out dan pembayaran selesai dalam satu proses', 'Validasi: kamar harus available untuk tanggal tersebut', 'Error message: \"Kamar tidak tersedia untuk tanggal yang dipilih\"', 'Edge case: guest ingin extended stay Ã¢â‚¬â€ cek availability'],
      resort: ['Guest booking kamar resort untuk liburan', 'Resepsionis check-in dan assign kamar sesuai preferensi', 'Guest menikmati fasilitas resort: kolam renang, spa, aktivitas', 'Staff menyediakan housekeeping dan room service', 'Guest check-out Ã¢â‚¬â€ semua tagihan diselesaikan', 'Validasi: jumlah tamu tidak boleh melebihi kapasitas kamar', 'Error message: \"Kamar penuh untuk tanggal tersebut\"', 'Edge case: guest sakit selama menginap Ã¢â‚¬â€ bantuan medis dipanggil'],
      help_desk: ['Customer dapat membuat tiket dengan kategori dan prioritas', 'Sistem assign tiket ke agent berdasarkan beban kerja', 'Agent merespon dan mengupdate status tiket', 'SLA dipantau untuk setiap tiket berdasarkan prioritas', 'Tiket ditutup setelah customer konfirmasi resolusi', 'Validasi: subject dan deskripsi tidak boleh kosong', 'Error message: \"Tiket dengan subjek yang sama sudah ada\"', 'Edge case: customer reopen tiket yang sudah closed Ã¢â‚¬â€ tiket original diaktifkan kembali'],
      loyalty_program: ['Member dapat mendaftar ke program loyalitas dengan data diri', 'Poin diperoleh dari setiap transaksi atau aktivitas tertentu', 'Member dapat menukarkan poin dengan reward yang tersedia', 'Tier member meningkat berdasarkan total poin atau transaksi', 'Poin memiliki masa berlaku dan akan expired jika tidak digunakan', 'Validasi: poin member harus cukup untuk menukar reward', 'Error message: \"Poin tidak mencukupi. Dibutuhkan {points} poin\"', 'Edge case: reward habis Ã¢â‚¬â€ tampilkan "stok habis" dan beri notifikasi saat tersedia'],
      sales_pipeline: ['Sales membuat lead baru dari berbagai sumber', 'Lead dikualifikasi melalui serangkaian aktivitas follow-up', 'Lead yang qualified dikonversi menjadi deal dengan value', 'Deal bergerak melalui pipeline stages hingga closing', 'Manager melihat forecast revenue berdasarkan pipeline', 'Validasi: value deal harus lebih dari 0', 'Error message: \"Lead sudah terdaftar dengan email yang sama\"', 'Edge case: deal loss di stage akhir Ã¢â‚¬â€ catat reason untuk analysis'],
      project_management: ['Manager membuat project dan menetapkan timeline', 'Task di-assign ke anggota tim dengan deadline dan prioritas', 'Anggota tim mengupdate status task dan mencatat hours', 'Milestone menjadi checkpoint progress project', 'Project selesai Ã¢â‚¬â€ semua deliverables terdokumentasi', 'Validasi: endDate harus setelah startDate', 'Error message: \"Tanggal selesai harus setelah tanggal mulai\"', 'Edge case: resource tidak tersedia Ã¢â‚¬â€ reassign task atau adjust timeline'],
      task_management: ['User membuat task baru dengan judul dan prioritas', 'Task dikelompokkan dalam list yang terorganisir', 'User dapat menambahkan label dan deadline', 'Task diupdate statusnya dari todo ke done', 'Task yang sudah lama selesai diarsipkan otomatis', 'Validasi: judul task tidak boleh kosong', 'Error message: \"Task gagal dibuat. Coba lagi\"', 'Edge case: user membuat subtask Ã¢â‚¬â€ gunakan parent-child relationship'],
      note_taking: ['User dapat membuat notebook untuk mengorganisir catatan', 'Catatan dibuat dengan rich text editor', 'Tag digunakan untuk mengkategorikan catatan', 'Pencarian full-text untuk menemukan catatan dengan cepat', 'Catatan dapat diarsipkan atau dihapus (soft delete)', 'Validasi: judul catatan tidak boleh kosong', 'Error message: \"Gagal menyimpan catatan\"', 'Edge case: 2 user edit catatan yang sama di waktu bersamaan Ã¢â‚¬â€ merge conflict handling'],
      okr_tracking: ['User atau manager menetapkan objective untuk periode tertentu', 'Setiap objective memiliki 3-5 key results yang terukur', 'Key result memiliki baseline, target, dan progress saat ini', 'Check-in dilakukan mingguan untuk update progress', 'Di akhir periode, skor OKR dihitung berdasarkan pencapaian KR', 'Validasi: target value harus di atas start value', 'Error message: \"Key result harus memiliki target yang terukur\"', 'Edge case: objective di tengah periode berubah prioritas Ã¢â‚¬â€ archive dan buat baru'],
      content_subscription: ['Creator membuat konten premium untuk subscriber', 'Plan langganan dibuat dengan harga dan benefit berbeda', 'Subscriber memilih plan dan membayar berlangganan', 'Konten eksklusif hanya bisa diakses subscriber aktif', 'Creator melihat analytics subscriber dan revenue', 'Validasi: pembayaran harus berhasil sebelum akses konten diberikan', 'Error message: \"Langganan tidak aktif. Perbarui pembayaran\"', 'Edge case: pembayaran gagal setelah akses diberikan Ã¢â‚¬â€ revoke akses hingga pembayaran sukses'],
      podcast_platform: ['Host merekam dan mengupload episode podcast', 'Episode dipublikasikan ke feed subscribers', 'Listener berlangganan podcast dan mendapatkan notifikasi episode baru', 'Setiap episode memiliki statistik: listens, durasi rata-rata, completion rate', 'Host mendapat revenue dari iklan atau donasi pendengar', 'Validasi: file audio harus dalam format yang didukung (MP3, AAC)', 'Error message: \"Format audio tidak didukung\"', 'Edge case: episode mengandung konten sensitif Ã¢â‚¬â€ beri peringatan sebelum diputar'],
      template_marketplace: ['Creator upload template dengan file dan preview', 'Template direview oleh admin sebelum dipublikasikan', 'Buyer mencari template berdasarkan kategori dan format', 'Pembelian memberikan akses download unlimited', 'Creator mendapat payout berdasarkan penjualan', 'Validasi: file template harus di bawah 100MB', 'Error message: \"File terlalu besar. Maksimal 100MB\"', 'Edge case: buyer tidak puas dengan kualitas Ã¢â‚¬â€ ajukan refund dalam 7 hari'],
      fishery: ['Petani menyiapkan kolam dan menebar benih ikan', 'Pakan diberikan secara terjadwal dengan jumlah yang tepat', 'Pertumbuhan ikan dimonitor secara berkala', 'Ikan dipanen saat mencapai berat target', 'Hasil panen dijual ke buyer dengan harga pasar', 'Validasi: jumlah pakan tidak boleh melebihi kapasitas kolam', 'Error message: \"Kolam tidak siap untuk penebaran\"', 'Edge case: ikan mati massal Ã¢â‚¬â€ investigasi penyebab dan catat kerugian'],
      plantation: ['Petani mempersiapkan lahan dan menanam bibit', 'Perawatan dilakukan secara rutin: pemupukan, penyiraman, pengendalian hama', 'Tanaman dipantau pertumbuhannya hingga siap panen', 'Panen dilakukan pada waktu yang tepat untuk kualitas optimal', 'Hasil panen dijual atau diolah lebih lanjut', 'Validasi: musim tanam harus sesuai dengan jenis tanaman', 'Error message: \"Bukan musim tanam yang tepat\"', 'Edge case: gagal panen karena cuaca buruk Ã¢â‚¬â€ klaim asuransi pertanian'],
      greenhouse: ['Petani menanam tanaman di lingkungan rumah kaca yang terkontrol', 'Sensor memonitor suhu, kelembaban, dan pH secara real-time', 'Sistem secara otomatis mengatur ventilasi dan irigasi', 'Tanaman dipanen saat mencapai kualitas prima', 'Hasil panen dijual langsung ke konsumen atau restoran', 'Validasi: sensor harus dikalibrasi secara berkala', 'Error message: \"Sensor tidak merespon. Periksa koneksi\"', 'Edge case: listrik padam Ã¢â‚¬â€ backup generator otomatis menyala'],
      car_wash: ['Customer datang dan memilih paket cuci yang diinginkan', 'Worker mencatat pesanan dan mulai proses pencucian', 'Mobil dicuci sesuai prosedur: exterior, interior, vacuum', 'Mobil dikeringkan dan diperiksa kualitasnya', 'Customer membayar dan menerima struk', 'Validasi: nomor plat kendaraan harus diisi', 'Error message: \"Nomor plat tidak valid\"', 'Edge case: customer request tambahan setelah cuci dimulai Ã¢â‚¬â€ sesuaikan harga dan waktu'],
      motorcycle_workshop: ['Customer datang dengan keluhan pada motornya', 'Mekanik mendiagnosa masalah dan memberikan estimasi biaya', 'Customer menyetujui estimasi Ã¢â‚¬â€ perbaikan dimulai', 'Spare part yang rusak diganti dengan yang baru', 'Motor selesai diperbaiki dan customer bayar', 'Validasi: estimasi biaya harus disetujui customer sebelum repair', 'Error message: \"Spare part tidak tersedia. Pesan terlebih dahulu\"', 'Edge case: biaya perbaikan membengkak Ã¢â‚¬â€ konfirmasi ulang ke customer'],
      tire_shop: ['Customer datang untuk cek kondisi ban', 'Teknisi memeriksa tekanan udara dan kedalaman alur ban', 'Ban yang aus diganti dengan yang baru sesuai ukuran', 'Ban baru di balancing dan spooring', 'Customer bayar dan kendaraan siap digunakan', 'Validasi: ukuran ban harus sesuai dengan spesifikasi kendaraan', 'Error message: \"Ukuran ban tidak tersedia\"', 'Edge case: customer mau ganti hanya 2 ban Ã¢â‚¬â€ infokan pentingnya keseimbangan ban'],
      rental_management: ['Owner mendaftarkan properti dan unit yang tersedia', 'Tenant menandatangani perjanjian sewa untuk unit tertentu', 'Pembayaran sewa dilakukan bulanan dan dicatat sistem', 'Perbaikan diajukan oleh tenant dan ditindaklanjuti owner', 'Kontrak sewa diperpanjang atau diakhiri sesuai ketentuan', 'Validasi: endDate lease harus setelah startDate', 'Error message: \"Unit sudah disewa untuk periode tersebut\"', 'Edge case: tenant telat bayar 3 bulan berturut-turut Ã¢â‚¬â€ proses eviction sesuai kontrak'],
      real_estate_agency: ['Agent mendaftarkan properti dengan foto dan detail lengkap', 'Client booking jadwal untuk melihat properti', 'Agent menunjukkan properti dan menjelaskan kelebihan', 'Buyer mengajukan penawaran harga Ã¢â‚¬â€ agent negosiasi', 'Deal ditutup Ã¢â‚¬â€ komisi agent dibayarkan', 'Validasi: harga listing harus realistis berdasarkan analisa pasar', 'Error message: \"Harga properti di luar estimasi\"', 'Edge case: buyer mundur setelah deal Ã¢â‚¬â€ deal jadi batal dan listing diaktifkan kembali'],
    ...
    if (engineArtifacts && engineArtifacts.stateMachine && engineArtifacts.stateMachine.stateMachines.length > 0) {
      ...
      engineArtifacts.stateMachine.stateMachines.forEach(sm => {
        enginePromptLines.push('- **' + sm.entity + '**: ' + sm.states.join(' Ã¢â€ â€™ '));
    ...
    const handoffRules = [
      ...
      '- Every result should reflect the userÃ¢â‚¬â„¢s actual intent, not generic AI output.',
    ...
    const promptContent = [
      ...
      '',
      '- **Nama**: ' + productName,
      '- **Domain**: ' + domain + (domainInfo.confidence > 0 ? ' (confidence: ' + (domainInfo.confidence * 100).toFixed(0) + '%)' : '') + (domainInfo.secondary.length ? ' Ã¢â‚¬â€ Related: ' + domainInfo.secondary.join(', ') : ''),
      '- **Ide**: ' + ideaText,
      ...
      '1. prisma/schema.prisma Ã¢â‚¬â€ database schema first',
      '2. backend/ Ã¢â‚¬â€ all backend code',
      '3. frontend/ Ã¢â‚¬â€ all frontend code',
      '4. tests/ Ã¢â‚¬â€ tests',
      '5. docker-compose.yml Ã¢â‚¬â€ deployment',
      '',
      '## 14. Priority Order',
      '',
      'Build in this priority order:',
      '',
      '**P0 Ã¢â‚¬â€ Core (build first):**',
      ...
      '',
      '**P1 Ã¢â‚¬â€ Essential (build second):**',
      '- UI pages (detail, search, filter)',
      '- Validation and error handling',
      '- Auth integration',
      '',
      '**P2 Ã¢â‚¬â€ Enhancement (build last):**',
      ...
      '',
      '### DO NOT:',
      '- Ã¢ÂÅ’ No gradient backgrounds or heavy drop-shadows',
      '- Ã¢ÂÅ’ No stock images or illustrations',
      '- Ã¢ÂÅ’ No rounded borders >12px',
      '- Ã¢ÂÅ’ No marketing/landing pages',
      '- Ã¢ÂÅ’ No "Under Construction" pages or placeholders',
      '- Ã¢ÂÅ’ No unused components or dead code',
      '',
      '### MUST HAVE:',
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ PRD content Ã¢â€â‚¬Ã¢â€â‚¬
    const prdContent = '# PRD - ' + productName + '\n\n' +
      ...
      'Operating rules:\n' +
      '- Start from the userÃ¢â‚¬â„¢s real workflow and preserve it end-to-end.\n' +
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ Output Summary Ã¢â€â‚¬Ã¢â€â‚¬
    const summaryContent = '# Ringkasan Output: ' + productName + '\n\n' +
      '## Cara Pakai\n' +
      '1. Buka `prompt.md` Ã¢â€ â€™ Copy semua konten\n' +
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ README Ã¢â€â‚¬Ã¢â€â‚¬
    const readmeContent = '# ' + productName + '\n\n' +
      ...
      '## Files\n' +
      '- `prompt.md` Ã¢â‚¬â€ Master prompt for AI tools (Claude Code, Codex, Cursor)\n' +
      '- `prd.md` Ã¢â‚¬â€ Full product requirements document (19 sections)\n' +
      '- `output-summary.md` Ã¢â‚¬â€ Quick overview\n\n' +
      '## Tech Stack\n' +
      '- Frontend: ' + tech.frontend + '\n' +
      '- Backend: ' + tech.backend + '\n' +
      '- Database: ' + tech.database + '\n' +
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ Set artifacts Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Chunked & Task Artifacts Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
async function enrichWithAI() {
  ...
  const enrichmentPrompt = [
    ...
    '',
    '1. Bagian "Detailed User Flow" Ã¢â‚¬â€ tambahkan 1-2 skenario spesifik dan realistis sesuai domain produk',
    '2. Bagian "Edge Cases" Ã¢â‚¬â€ tambahkan 3-5 edge cases spesifik yang benar-benar terjadi di domain ini',
    '3. Bagian "Error Handling" Ã¢â‚¬â€ tambahkan error recovery strategy spesifik sesuai tech stack',
    '4. Bagian "Build Plan" Ã¢â‚¬â€ bagi menjadi sprint yang lebih realistis dengan deliverables konkret',
    '5. Bagian "API Contract" Ã¢â‚¬â€ tambahkan endpoint spesifik yang dibutuhkan domain ini',
    '',
...
function resetState() {
  ...
  state.tech = { frontend: 'ai-pilih', backend: 'ai-pilih', database: 'ai-pilih', deployment: 'ai-pilih' };
  state.extras = [];
  state.answers = {};
  state.artifacts = [];
  state.currentArtifact = 0;
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Artifact Rendering Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function renderArtifacts() {
  ...
  if (tabs) {
    ...
    // Group definitions Ã¢â‚¬â€ Workspace, Documents, Project (metadata), then Engine
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ View Mode: Preview vs Edit Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Preview Pane Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Version Rendering Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Meta (kept for compatibility, renders into a hidden element if needed) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function renderMeta() {
  // No separate meta section in new layout; kept as no-op for compatibility
}

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ History Sidebar Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Revision Chat Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Download Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ AI Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬

// Generate AI ideas in wizard page based on product name Ã¢â‚¬â€ NO static fallback
async function generateAIExampleIdeas() {
  ...
  try {
    ...
    const prompt = [
      ...
      '',
      'Buat 3 IDE PRODUK DIGITAL yang SPESIFIK Ã¢â‚¬â€ aplikasi/platform/software',
      ...
      '- REALISTIS untuk UKM/startup Indonesia Ã¢â‚¬â€ bisa dibangun tim 1-3 developer',
      ...
      '',
      'Contoh SALAH Ã¢â‚¬â€ JANGAN:',
      '  "Laundry Surprise" Ã¢â‚¬â€ nama brand extension, BUKAN produk digital utuh',
      '  "Laundry Mystery Box" Ã¢â‚¬â€ ini bukan aplikasi/platform, cuma ide model bisnis',
      '  "Fitur Tracking Driver" Ã¢â‚¬â€ ini fitur, BUKAN produk utuh',
      '',
    ...
    // Graceful message Ã¢â‚¬â€ no scary errors, just gentle push to manual
    wizIdeas.innerHTML =
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ AI Processing UI Helper Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ AI Tech Recommendation Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
async function generateAITechRecommendation() {
  ...
  // Show loading state in tech subcards area
  var sc = document.getElementById('techSubcards');
  if (sc) {
    sc.innerHTML = '<div class="flex flex-col items-center py-6 gap-3"><div class="loading-ai"><div class="spinner-ring"></div></div><div class="text-[12px] animate-pulse" style="color:var(--text-muted)">AI merekomendasikan teknologi...</div></div>';
  }
  // Rainbow glow on parent card while processing
  var techCard = document.querySelector('#page-wizard .card');
  if (techCard) techCard.classList.add('ai-processing');

  const catParts = [];
  if (state.productCategoryParent) catParts.push(state.productCategoryParent);
  if (state.productCategory) catParts.push(state.productCategory);
  const categoryHint = catParts.length ? catParts.join(' Ã¢â€ â€™ ') : '-';
  ...
  try {
    ...
    // Don't auto-fill â€” user chooses manually from accordion cards
...
function previewAIExampleIdea(idx) {
  ...
  // Check if already showing this idea Ã¢â‚¬â€ toggle off
...
function applyAIExampleIdea(idx) {
  ...
  if (ideaText) {
    ideaText.value = (idea.title || '') + ' Ã¢â‚¬â€ ' + (idea.desc || '');
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ AI Tech Apply Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
function applyAITechRec(cat) {
  ...
  var recId = techNameToId(cat, aiRec[cat].rec);
  if (recId === 'ai-pilih') return;
  window._pendingTech = window._pendingTech || {};
  window._pendingTech[cat] = recId;
  // Auto-confirm
  state.tech[cat] = recId;
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Provider List Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
const PROVIDER_LIST = [
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Settings Modal (Provider Selector UI) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
function renderSavedProvidersDropdown() {
  ...
  select.innerHTML = '<option value="">Ã¢â‚¬â€ Pilih provider tersimpan Ã¢â‚¬â€</option>';

  // Also populate custom dropdown
  const customPanel = document.getElementById('savedProviderDropdown');
  if (customPanel) {
    customPanel.innerHTML = '<div class="custom-dropdown-item empty" data-value="">Ã¢â‚¬â€ Pilih provider tersimpan Ã¢â‚¬â€</div>';
  }
  ...
  savedProviders.forEach((sp) => {
    ...
    const label = sp.name + (model ? ' Ã¢â‚¬â€ ' + model : '');
  ...
  // onchange handler Ã¢â‚¬â€ populate hidden fields for simpanPengaturan
...
function loadSavedProvider() {
  // Legacy Ã¢â‚¬â€ hidden fields populated by select.onchange above.
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Hapus Provider (D1-centric) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
async function hapusProvider() {
  ...
  try {
    ...
    // Jika yang dihapus adalah active_provider Ã¢â€ â€™ reset
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Add Provider Form Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Load Saved Providers from D1 Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Delete Saved Provider from D1 Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
async function verifyApiKey() {
  ...
  try {
    ...
    // Success Ã¢â‚¬â€ populate model dropdown
    const modelSelect = document.getElementById('settingsAiModel');
    const modelGroup = document.getElementById('settingsModelGroup');
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Utils Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Theme (subtle Ã¢â‚¬â€œ ensures body class matches mode) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function applyTheme() {
  document.body.dataset.mode = state.mode;
}

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Project History (persisted via D1) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
function renderResultOverview() {
  ...
  const archPattern = engineArch ? engineArch.pattern : 'Ã¢â‚¬â€';
  const archLayers = engineArch && engineArch.layers ? engineArch.layers.join(', ') : 'Ã¢â‚¬â€';
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ BUILD HTML Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  var html = '';
    ...
    '<div class="result-hero-inner">' +
      ...
      '<div class="result-hero-title-row">' +
        ...
        '<div class="result-hero-info">' +
          ...
          '<p class="result-hero-subtitle">' + escapeHtml(domainName) + ' Ã¢â‚¬Â¢ ' + entityCount + ' entitas Ã¢â‚¬Â¢ ' + moduleCount + ' modul</p>' +
        '</div>' +
      '</div>' +
      '<div class="result-hero-meta">' +
        '<div class="result-hero-meta-item">' + ovIcon('users') + '<span class="rhm-label">Aktor</span><span class="rhm-value">' + (actors.length ? escapeHtml(actors.join(', ')) : 'Ã¢â‚¬â€') + '</span></div>' +
  ...
  // Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â STATS DASHBOARD Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â
  var stats = [
  ...
  // Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â INSIGHT GRID (Architecture, Security, Lifecycle, Events) Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â
  ...
  html += '<div class="insight-card" style="--insight-accent:#3B82F6" ' + stagger(7) + '>' +
    ...
    '<div class="insight-body">' +
      '<div class="insight-row"><span class="insight-row-label">Autentikasi</span><span class="insight-tag">' + escapeHtml(authType || 'Ã¢â‚¬â€') + '</span></div>' +
      '<div class="insight-row"><span class="insight-row-label">MFA</span><span class="insight-tag">' + escapeHtml(mfa || 'Ã¢â‚¬â€') + '</span></div>' +
  ...
  // Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â UI FLOWS Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â
  if (journeys.length) {
  ...
  // Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â TECH STACK Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â
  ...
  // Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â MODULES Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â
  if (modules.length) {
  ...
  // Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â VALIDATION RULES Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â
  ...
  // Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â DOWNLOADS Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â
  ...
  var downloads = [
    { icon: 'fileText', label: 'Dokumen PRD', desc: 'Markdown Ã¢â‚¬â€ product requirements document', color: '#22C55E', action: 'downloadWorkspace()' },
    { icon: 'code', label: 'JSON Engine', desc: 'JSON Ã¢â‚¬â€ full engine analysis data', color: '#3B82F6', action: 'downloadAllJson()' },
...
function renderVisualTab() {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬ Entity Relationship Map Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  relations.forEach(function(r) {
    ...
    ['belongsTo', 'hasMany', 'hasOne'].forEach(function(type) {
      ...
      items.forEach(function(item) {
        var arrow = type === 'belongsTo' ? 'Ã¢â€ Â belongsTo' : type === 'hasMany' ? 'Ã¢â€ â€™ hasMany' : 'Ã¢â‚¬â€ hasOne Ã¢â€ â€™';
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬ State Machine Visuals Ã¢â€â‚¬Ã¢â€â‚¬
  if (stateMachines.length) {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬ User Flow Journeys Ã¢â€â‚¬Ã¢â€â‚¬
  if (journeys.length) {
...
function renderExportTab() {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬ Version History Ã¢â€â‚¬Ã¢â€â‚¬
  if (state.versions && state.versions.length > 0) {
...
function downloadWorkspace() {
  ...
  parts.push('> Domain: ' + ((window.engineArtifacts && window.engineArtifacts.domain) ? window.engineArtifacts.domain.primaryDomain : 'Ã¢â‚¬â€'));
  parts.push('');
  parts.push('---');
  parts.push('');
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Provider Radio List (Hermes CLI Style) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Select Provider Radio Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Filter Provider List Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function filterProviderList(value) {
  renderProviderRadioList(value);
}

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Open Provider Selector Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Cancel Add Provider Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Continue to Detail Form Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function continueToDetailForm() {
  if (selectedProvider) {
    document.getElementById('detailBaseUrl').value = selectedProvider.baseUrl || '';
    var icons = {'anthropic':'Ã°Å¸Â¤â€“','openai':'Ã°Å¸Å¸Â¢','openrouter':'Ã°Å¸â€â‚¬','gemini':'Ã°Å¸â€Âµ','sumopod':'Ã¢Å¡Â¡'};
    document.getElementById('detailProviderIcon').textContent = icons[selectedProvider.type] || 'Ã°Å¸â€Å’';
    document.getElementById('detailProviderDesc').textContent = (selectedProvider.type || 'custom') + ' Ã‚Â· verify API Key untuk lihat model';
  }
  syncDetailToHidden();
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Toggle API Key Visibility Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function toggleDetailApiKey() {
  const input = document.getElementById('detailApiKey');
  if (input) input.type = input.type === 'password' ? 'text' : 'password';
}

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Verify API Key (Detail Form) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
async function detailVerifyKey() {
  ...
  try {
    ...
    // Success Ã¢â‚¬â€ populate model dropdown (native + custom)
    ...
    document.getElementById('verifiedProviderName').textContent = provName ? provName.textContent : 'Ã¢â‚¬â€';
    document.getElementById('providerDetailStep').style.display = 'none';
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Simpan Provider (Add new to D1) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Simpan Pengaturan (Set active provider from D1) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Beta Feedback Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
function showBetaDashboard() {
  ...
  console.log('Ã¢â€¢â€Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢â€”');
  console.log('Ã¢â€¢â€˜    PRDKit Beta Dashboard     Ã¢â€¢â€˜');
  console.log('Ã¢â€¢Â Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â£');
  console.log(`Ã¢â€¢â€˜ Generations: ${gens.length} total, ${fails.length} failed`);
  console.log(`Ã¢â€¢â€˜ Feedback: ${fb.length} submissions`);
  console.log(`Ã¢â€¢â€˜ Unknown domains: ${unknown.length} pending`);
  console.log(`Ã¢â€¢â€˜ Avg confidence: ${(avgConf * 100).toFixed(0)}%`);
  console.log('Ã¢â€¢Â Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â£');
  console.log('Ã¢â€¢â€˜ Top domains:');
  topDomains.forEach(([d, c]) => console.log(`Ã¢â€¢â€˜   ${d.padEnd(20)} ${c}`));
  console.log('Ã¢â€¢Å¡Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â');
}
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Business DNA Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
window.showBetaDashboard = showBetaDashboard;
  ...
  /* ========================================
     PRDKit Ã¢â‚¬â€ Page Logic & Scripts
     ======================================== */

  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Wizard Page State Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  let wizardStep = 1;
  let wizardSurveyQ = 0;
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Setup State Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  let selectedCategory = null;
  let selectedParent = null;
  let selectedType = 'Web App';

  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Hash helpers Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Category Translation (English Ã¢â€ â€™ Indonesian) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  var CAT_INDONESIA = {
    'Products': 'Produk',
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Tech Stack by Product Type Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  var TECH_BY_TYPE = {
    'Web App': [
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Smart Tech Recommendation Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  function applyAIRecommendation() {
    ...
    if (aiChips) {
      aiChips.innerHTML = '<span class="chip selected" style="color:var(--accent);background:var(--accent-soft);border-color:rgba(0,224,143,0.25);cursor:default">Ã¢Å“â€¦ ' + rec.tech + '</span><span class="text-[10px] ml-1" style="color:var(--text-muted)">' + rec.reason + '</span>';
    }
  }

  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Setup Page Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  function initSetup() {
    // Clear stale state from previous sessions (localStorage) Ã¢â‚¬â€ setup always starts fresh
    if (typeof state !== 'undefined') {
      state.productName = '';
      state.idea = '';
      state.productCategory = '';
  ...
  function renderCategory() {
    ...
    CATEGORIES.forEach(function(cat) {
      ...
      card.innerHTML = '<div class="cat-card-inner"><div class="cat-icon">' + (cat.icon ? svgIcon(cat.icon) : 'Ã°Å¸â€œÂ') + '</div><div class="text-[11px] sm:text-[12px] font-semibold text-center" style="color:var(--text)">' + displayName + '</div></div>';
  ...
  function selectParent(name) {
    ...
    if (selectedParent) {
      ...
      if (subAreaCompat) {
        ...
        subAreaCompat.innerHTML = '<div style="padding:8px 0"><span style="color:var(--accent);font-size:12px">' + id(selectedParent) + ' Ã¢â€“Â¸ ' + (selectedCategory ? id(selectedCategory) : 'pilih subkategori') + '</span></div>';
      }
    } else {
      grid.style.display = '';
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Category type chips Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Terminal tabs Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  document.addEventListener('DOMContentLoaded', function() {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Settings Modal Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Update AI Config Indicator on Home Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  if (typeof updateAIConfigUI !== 'function') {
    window.updateAIConfigUI = function() {
      ...
      if (configured) {
        ...
        indicator.innerHTML = '<span class="ai-config-icon">Ã¢â€”Â</span><div class="ai-config-text"><span onclick="openSettings()">' + state.aiModel + '</span></div>';
      } else {
        createBtn.disabled = true;
        modelBtn.classList.add('warn-outline');
        indicator.className = 'ai-config-indicator warn mb-3';
        indicator.innerHTML = '<span class="ai-config-icon">Ã¢Å¡Â </span><div class="ai-config-text"><span>AI models belum dikonfigurasi.</span><span class="ai-config-sub">Konfigurasikan model terlebih dahulu untuk mulai membuat blueprint.</span></div>';
      }
    };
  }

  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Override renderSavedProvidersDropdown to handle noConfigMsg + populate hidden fields Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  renderSavedProvidersDropdown = function() {
    ...
    // When user selects a saved provider Ã¢â€ â€™ populate hidden fields for saveSettings()
    if (select) {
      select.onchange = function() {
        var delBtn = document.getElementById('deleteProviderBtn');
        if (delBtn) delBtn.disabled = !this.value;
  ...
  window.simpanPengaturan = async function simpanPengaturanFn() {
    ...
    // Langsung fetch dari D1 Ã¢â‚¬â€ ga pake saveSettings() atau DOM sama sekali
    try {
      var API_BASE = typeof API_URL !== 'undefined' ? API_URL : 'https://prdkit-ai-proxy.halugoodsindonesia.workers.dev';
      ...
      // Extract model Ã¢â‚¬â€ handle berbagai format
      var model = '';
      if (sp.models && Array.isArray(sp.models) && sp.models.length > 0) {
        model = sp.models[0];
      } else if (sp.model) {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Custom Dropdown Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Render Provider List (Step 1 Ã¢â‚¬â€ searchable radio list) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  function renderProviderList(query) {
    ...
    filtered.forEach(function(p, i) {
      ...
      card.onclick = function() {
        ...
        document.getElementById('detailProviderDesc').textContent = (p.type || 'custom') + ' Ã‚Â· verify API Key untuk lihat model';
      ...
      var iconEl = p.type === 'anthropic' ? 'Ã°Å¸Â¤â€“' : p.type === 'openai' ? 'Ã°Å¸Å¸Â¢' : p.type === 'openrouter' ? 'Ã°Å¸â€â‚¬' : p.type === 'gemini' ? 'Ã°Å¸â€Âµ' : p.type === 'sumopod' ? 'Ã¢Å¡Â¡' : 'Ã°Å¸â€Å’';
      var subtitle = (p.type || 'custom') + (p.baseUrl ? ' Ã‚Â· ' + p.baseUrl.replace('https://','').split('/')[0] : '') + ' Ã‚Â· verify utk model';
      card.innerHTML = '<div class="cat-card-inner" style="height:auto;padding:12px;flex-direction:row;justify-content:flex-start;gap:12px">' +
        ...
        (isSelected ? '<div style="color:var(--accent);font-size:14px;flex-shrink:0">Ã¢Å“â€œ</div>' : '') +
        '</div>';
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Wizard Page Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  function initWizard() {
    wizardStep = 1;
    ...
    if (state) {
      if (state.savedQuestions && Array.isArray(state.savedQuestions) && state.savedQuestions.length > 0) {
        // Restore persisted questions Ã¢â‚¬â€ keep surveyMode so survey renders directly
        _surveyQuestions = state.savedQuestions;
        state.surveyTotal = _surveyQuestions.length;
      } else {
        // No saved questions Ã¢â‚¬â€ reset to mode picker
        state.surveyTotal = 0;
        state.surveyMode = '';
        _surveyQuestions = [];
      }
  ...
  function renderWizStep() {
    ...
    if (wizardStep === 1) { container.innerHTML = renderStep1HTML(); }
      ...
      // AI survey: auto-regenerate questions if mode selected but empty
      if (state.surveyMode) {
        if (_surveyQuestions && _surveyQuestions.length > 0) {
          setTimeout(function() {
            if (typeof renderSurvey === 'function') renderSurvey();
          }, 100);
        } else {
          // Regenerate silently in background
          setTimeout(function() {
            var loading = document.getElementById('surveyLoading');
            if (loading) loading.style.display = 'block';
            generateSurveyQuestions(state.surveyMode).then(function() {
              renderWizStep();
            });
          }, 100);
        }
  ...
  function renderStep1HTML() {
    ...
    return '<div class="stagger">' +
      ...
      // Card 1: Contoh Ide / AI recommendations Ã¢â‚¬â€ FIRST
      ...
      // Card 2: Idea textarea + Kembangkan dengan AI Ã¢â‚¬â€ SECOND
      ...
      '<div></div>' +
      '<button class="btn-primary rounded-xl font-semibold text-sm" onclick="goWizardStep(2)" style="border:none">Lanjut ke Survey Ã¢â€ â€™</button>' +
      '</div></div>';
  }
...
function fillWizIdea(nama, ideaText) {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Tech Selection Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  function selectTech(el) {
    ...
    if (aiChips) {
      ...
      aiChips.innerHTML = '<span class="chip selected" style="color:var(--text-muted);background:rgba(255,255,255,0.04);border-color:var(--border);cursor:default">Ã°Å¸â€Â§ ' + escapeHtml(selectedTech) + '</span><span class="text-[10px] mt-1" style="color:var(--text-muted)">' + desc + '</span>';
    }
  }

  Ã¯Â»Â¿// Global techNameToId (hoisted from generateAITechRecommendation)
function techNameToId(cat, name) {
  if (!name) return 'ai-pilih';
  var opts = TECH_OPTIONS[cat] || [];
  var n = (name || '').toLowerCase().trim();
...
// Toggle tech accordion card Ã¢â‚¬â€ only one open at a time
...
// Select tech for a category Ã¢â‚¬â€ mark pending, not locked
function selectTechSub(cat, val, el) {
  window._pendingTech = window._pendingTech || {};
  window._pendingTech[cat] = val;
...
// Confirm pending tech selection
...
// Edit already-confirmed tech selection
function editTechSelection(cat) {
  state.tech[cat] = 'ai-pilih';
  if (typeof saveState === 'function') saveState();
...
function renderTechSubcardsHTML() {
  ...
  cats.forEach(function(cat) {
    ...
    html += '<div class="tech-accord-card' +
      ...
      // Ã¢â€â‚¬Ã¢â€â‚¬ Header (always visible) Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    if (isConfirmed) {
      var confirmedLabel = escapeHtml(findLabel(confirmedVal));
      ...
      var pendingLabel = escapeHtml(findLabel(pendingVal));
      html += '<span class="tech-accord-status">' + pendingLabel + '</span>';
    } else {
      html += '<span class="tech-accord-status">Pilih Ã¢â€“Â¼</span>';
    }

    if (!isConfirmed) {
      html += '<span class="tech-accord-arrow">' + iconSvg('arrowRight', 12) + '</span>';
    }

    html += '</div>'; // end header

    // Ã¢â€â‚¬Ã¢â€â‚¬ Body (expandable) Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    optsList.forEach(function(opt) {
      ...
      var sel = activeVal === opt[0] ? ' selected' : '';
      var isAiRec = false;
...
// â”€â”€â”€ AI Tech Recommendation Panel (rendered in separate card) â”€â”€â”€
function renderAITechContainer() {
  ...
  if (aiRec && aiRec.frontend && aiRec.frontend.rec) {
    ...
    cats.forEach(function(cat) {
      ...
      var recId = techNameToId(cat.id, recData.rec);
      var isApplied = state.tech[cat.id] && state.tech[cat.id] === recId;
    ...
    // Still loading
    container.innerHTML = '<div class="ai-rec-loading" style="padding:8px 0;display:flex;align-items:center;gap:8px">' +
      '<span class="spinner-ring" style="display:inline-block;width:10px;height:10px"></span>' +
      '<span class="text-[11px]" style="color:var(--text-muted)">AI merekomendasikan teknologi...</span></div>';
  } else {
    // No rec yet â€” show generate button
...
Ã¯Â»Â¿function renderStep2HTML() {
    if (typeof state !== 'undefined') {
      state.tech = state.tech || { frontend: 'ai-pilih', backend: 'ai-pilih', database: 'ai-pilih', deployment: 'ai-pilih' };
      state.extras = state.extras || [];
    }
    return '<div class="stagger">' +

      // Card 1: AI Recommendation (small helper card)
      '<div class="stagger" style="animation-delay:0.05s"><div class="card" style="--accent:#A855F7"><div class="card-inner !p-3 sm:!p-4">' +
      '<h3 class="text-[11px] font-semibold mb-1" style="color:var(--text-muted)">' + iconSvg('zap', 10) + ' Rekomendasi AI</h3>' +
      '<p class="text-[10px] mb-1" style="color:var(--text-muted)">Dapatkan rekomendasi teknologi dari AI berdasarkan ide &amp; survey kamu.</p>' +
      '<div id="aiTechRecContainer"></div>' +
      '</div></div></div>' +

      // Card 2: Tech Selection (big main card)
      '<div class="stagger mt-3 sm:mt-4" style="animation-delay:0.12s"><div class="card" style="--accent:#3B82F6"><div class="card-inner !p-5 sm:!p-7">' +
      ...
      '<div class="mt-5"><h3 class="text-[13px] font-semibold mb-1" style="color:var(--text)">Extra Features</h3>' +
      '<p class="text-[10px] mb-2" style="color:var(--text-muted)">Fitur tambahan yang relevan dengan produkmu.</p>' +
      '<div class="flex flex-wrap gap-2" id="extraChips"></div></div>' +
      '</div></div></div>' +
      '<div class="wizard-nav">' +
      '<button class="btn-ghost rounded-xl font-semibold text-sm" onclick="goWizardStep(2)">â† Kembali</button>' +
      '<button class="btn-primary rounded-xl font-semibold text-sm" id="goStep4Btn" onclick="goStep4FromTech()" style="border:none">Lanjut ke Blueprint â†’</button>' +
      '</div>';
  }
...
function renderStep3HTML() {
    // If no survey mode selected yet, show mode picker
      ...
      return '<div class="stagger"><div class="card" style="--accent:#22C55E"><div class="card-inner !p-4 sm:!p-6">' +
        ...
        '</div></div></div>' +
        '<div class="wizard-nav">' +
        '<button class="btn-ghost rounded-xl font-semibold text-sm" onclick="goWizardStep(1)">Ã¢â€ Â Kembali</button>' +
        '<div></div></div>';
    } else { // mode selected Ã¢â‚¬â€ show questions
    return '<div class="stagger"><div class="card" style="--accent:#22C55E"><div class="card-inner !p-4 sm:!p-6">' +
      ...
      '<div class="wizard-nav">' +
      '<button class="btn-ghost rounded-xl font-semibold text-sm" onclick="goWizardStep(1)">Ã¢â€ Â Kembali</button>' +
      '<button class="btn-primary rounded-xl font-semibold text-sm" onclick="surveyGoNext()" style="border:none">Lanjut Ã¢â€ â€™</button>' +
      '</div>';
  }
}

// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Step 4: Result / Review Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
function renderStep4HTML() {
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ Product Summary Card Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ Survey Stats Card Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ Tech Stack Card Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    html += '<div class="wizard-nav stagger mt-5" style="animation-delay:0.28s">' +
        '<button class="btn-ghost rounded-xl font-semibold text-sm" onclick="goWizardStep(3)">Ã¢â€ Â Kembali</button>' +
...
function goStep4FromTech() {
  ...
  function generateBtnClick() {
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Save all context to state Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    if (typeof state !== 'undefined') {
  ...
  function loadWizardOverlay() {
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Lab Animation Sequence Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    // Phase 1Ã¢â€ â€™2: Cards combine
    setTimeout(function() {
      if (p1) p1.style.display = 'none';
      if (p2) p2.style.display = 'block';
    }, 2200);

    // Phase 2Ã¢â€ â€™3: Beaker appears
    setTimeout(function() {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Engine Artifacts Reconstruction Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Result page Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  function initResult() {
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Docs & Tutorial Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  function showToast(msg, type) {
    ...
    var el = document.createElement('div');
    el.className = 'toast' + (type === 'success' ? ' success' : '') + (type === 'error' ? ' error' : '') + (type === 'info' ? ' info' : '') + (type === 'warning' ? ' warning' : '');
    el.innerHTML = '<span class="toast-icon">' + (type === 'success' ? 'Ã¢Å“â€œ' : type === 'error' ? 'Ã¢Å“â€¢' : type === 'warning' ? 'Ã¢Å¡Â ' : 'Ã¢â€žÂ¹') + '</span><span class="toast-msg">' + msg + '</span>';
    container.appendChild(el);
    setTimeout(function() { el.style.animation = 'toastOut 0.2s forwards'; setTimeout(function() { el.remove(); }, 250); }, 2500);
  }

  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Survey Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  function loadSurveyRecommendations() {
    try {
      ...
      if (qs.length > 0) {
        ...
        if (recCont) recCont.innerHTML = '<span class="chip" style="color:var(--accent)">Saran siap Ã¢â‚¬â€ lanjut generate!</span>';
      }
    } catch(e) {}
  }
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ createToast fallback (for app.js logout) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Language Toggle Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  var currentLang = 'id';
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ DOMContentLoaded Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  document.addEventListener('DOMContentLoaded', function() {
